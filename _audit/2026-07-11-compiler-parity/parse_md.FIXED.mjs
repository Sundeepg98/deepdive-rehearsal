...
function parseBank(toks) {
  ...
  const mkBeat = (raw, m) => {
    const p = raw.split(' | ');
    if (m === 'curve') {
      const q = p[0].trim().toUpperCase() === 'CURVEBALL' ? p.slice(1) : p;   /* FIX: drop the literal tag token */
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    if (t.type === 'paragraph_open' && beat) {
      const raw = toks[i + 1].content;
      /* FIX: fields are LINES inside one paragraph (as TOPIC_MARKDOWN_FORMAT.md:379-382
         demonstrates), not one field per paragraph. Split on the labels, keep the
         continuation lines with their field (Int/Int2 need the answer line). */
