...
// ---------------------------------------------------------------------------
// PARITY FIX -- the ONE root cause behind 531 dropped items.
//
// markdown-it merges soft-wrapped consecutive lines into ONE paragraph token whose
// .content carries the newlines. Every pane parser below treated a paragraph as a
// SINGLE record: it regex-matched a "Label:" prefix and then took raw.slice(...) --
// swallowing every following line into the first field. And the two list-shaped
// sections (sys stages, drill tier-notes) only accepted bullet_list tokens.
//
// TOPIC_MARKDOWN_FORMAT.md demonstrates the PLAIN-LINE form in both cases
// (:186-190 tier notes, :249-253 stages, :261-262 pivot chip, :378-381 bank beat),
// and all 38 authors followed the doc. The parser could not read its own spec.
//
// lines(raw)     -- a paragraph's logical records.
// segment(raw,re)-- split a paragraph into [{key,body}] on "Label:" prefixes, so a
//                   continuation line stays attached to the field that owns it.
// ---------------------------------------------------------------------------
...
function parseSys(toks) {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    if (t.type === 'paragraph_open') {
      ...
      // FIX 1: stages authored as PLAIN LINES (FORMAT.md:249-253) land here as one
      // paragraph, and the old code had no branch for them -> all 189 silently dropped.
      else if (mode === 'stages') {
        for (const ln of lines(raw)) {
          const cur = /\[\*\]\s*$/.test(ln);
          const s = ln.replace(/\s*\[\*\]\s*$/, '');
          const k = s.indexOf(': ');
          const stage = { n: prose(k === -1 ? s : s.slice(0, k)), d: prose(k === -1 ? '' : s.slice(k + 2)) };
          if (cur) stage.cur = true;
          stages.push(stage);
        }
      }
        ...
        const m = /^(->|\u2192)\s*/.exec(raw);
        // FIX 2: the chip and its answer are consecutive LINES (FORMAT.md:261-262), so
        // raw.slice() swallowed the answer into the chip -> 76 pivot answers lost.
        if (m && !piv.chip) {
          const rest = raw.slice(m[0].length);
          const nl = rest.indexOf('\n');
          piv.chip = '\u2192 ' + (nl === -1 ? rest : rest.slice(0, nl)).trim();
          if (nl !== -1) piv.a = prose(rest.slice(nl + 1).trim());
...
function parseDrill(toks) {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    // FIX 3: tier notes authored as PLAIN LINES (FORMAT.md:186-190) arrive as a paragraph
    // before the first card; the old code only read bullet_list_open -> all 114 dropped.
    if (t.type === 'paragraph_open' && !seenCard) {
      for (const ln of lines(toks[i + 1].content)) {
        const k = ln.indexOf(' | ');
        if (k !== -1) tierNotes[ln.slice(0, k).trim()] = prose(ln.slice(k + 3));
      }
      i += 2; continue;
    }
    if (t.type === 'paragraph_open' && card) {
      // FIX 4: Senior:/Speak: on consecutive lines (FORMAT.md:201-202) are ONE paragraph, so
      // `senior` swallowed the Speak: line. Segment the paragraph and keep each field's
      // continuation lines with it (Follow:'s answer line stays with Follow:).
      for (const s of segment(toks[i + 1].content, /^(follow|senior|speak)\s*:\s*/i)) {
        if (!s.key) {
          if (!s.body) continue;
          if (expectFollowA) { expectFollowA.a = prose(s.body); expectFollowA = null; }
          else if (!card.q) card.q = prose(s.body);
          else if (!card.a) card.a = prose(s.body);
        } else if (s.key === 'follow') {
          const nl = s.body.indexOf('\n');
          const fu = { q: prose(nl === -1 ? s.body : s.body.slice(0, nl)), a: prose(nl === -1 ? '' : s.body.slice(nl + 1)) };
          card.f.push(fu); expectFollowA = nl === -1 ? fu : null;
        } else if (s.key === 'senior') { card.senior = prose(s.body); expectFollowA = null; }
        else if (s.key === 'speak') { speak[speak.length - 1] = prose(s.body); expectFollowA = null; }
      }
      i += 2; continue;
    }
  }
...
function parseBank(toks) {
  ...
  const mkBeat = (raw, m) => {
    const p = raw.split(' | ');
    // FIX 5: the heading is "CURVEBALL | <theme> | <cue>" (FORMAT.md:386), so p[0] is the
    // literal tag -- the old code assigned it as the THEME, junking all 38 curveball themes
    // and shifting the real theme into the cue.
    if (m === 'curve') {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    if (t.type === 'paragraph_open' && beat) {
      // FIX 6: Task:/Model:/Int:/Int2: are consecutive LINES of ONE paragraph
      // (FORMAT.md:378-381). The old code matched ^task: then raw.slice() -- so `task`
      // swallowed Model:, Int: and the Int answer. 76 models + 76 int q/a lost.
      // int2 is tested before int because /^int:/ would also prefix-match "Int2:".
      for (const s of segment(toks[i + 1].content, /^(task|model|int2|int)\s*:\s*/i)) {
        if (s.key === 'task') beat.task = prose(s.body);
        else if (s.key === 'model') beat.model = prose(s.body);
