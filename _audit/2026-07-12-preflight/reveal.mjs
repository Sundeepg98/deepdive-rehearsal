  ...
  for (const theme of ['light', 'dark']) {
    for (const [g, topic] of Object.entries(TOPICS)) {
      ...
      const h = await p.evaluateHandle(() => {
        const all = [...document.querySelectorAll('*')].filter(e => e.children.length === 0 || e.tagName === 'BUTTON');
        return all.find(e => /^\s*Reveal answer\s*$/i.test(e.textContent || '') && e.getBoundingClientRect().width > 100) || null;
      });
      const el = h.asElement();
      if (!el) { rows.push({ build, theme, group: g, MISSING: true }); await h.dispose(); continue; }
      const meta = await el.evaluate(e => { const c = getComputedStyle(e); const r = e.getBoundingClientRect(); return { tag: e.tagName, cls: e.className.toString().slice(0, 30), grad: /gradient/.test(c.backgroundImage), fs: c.fontSize, fw: c.fontWeight, box: `${Math.round(r.width)}x${Math.round(r.height)}` }; });
...
console.log('=== "Reveal answer" PRIMARY CTA — pixel-sampled ===\n');
for (const b of ['BEFORE', 'AFTER']) for (const t of ['light', 'dark']) {
  ...
  console.log(`--- ${b} / ${t} ---`);
  r0.forEach(r => console.log(`  ${r.group.padEnd(26)} ${String(r.ratio).padStart(5)}:1 ${r.AA ? 'PASS' : 'FAIL'}  ${r.grad ? 'gradient' : 'flat    '}  text=${rgb(r.text).padEnd(18)} slab=${rgb(r.slab).padEnd(18)} ${r.box} ${r.fs}/${r.fw}`));
}
const f = rows.filter(r => !r.MISSING && !r.AA);
console.log(`\n>>> FAILING AA: ${f.length} of ${rows.filter(r => !r.MISSING).length}`);
