...
> ## ⚠ CORRECTIONS (dated 2026-07-19, from `perf/chunk-proto` round 2 — the original below stands as the historical verdict)
> Round-2 measurement on the post-merge base supersedes three specific claims:
>
> **1. CANDIDATE #1 STATUS.** This report calls candidate #1 (forced-reflow batching) "NOT prototyped this track" / "untouched" (§1 top-3 list, §4). CORRECTION: it was CODED in round-1's uncommitted tree — all four sites (router `pinTop` write-only, scroll-to-top cached `scrollY`, cram-overlay `.open`, shell `upd()` headin via captured CSSAnimation) — but UNVERIFIED and UNMEASURED (the cold verify measured only candidate #2, the deferral). It rode in the same tree; this audit didn't notice it. Round 2 split it into its own commit (af49426) and measured it in isolation.
>
> **2. CANDIDATE #1 MAGNITUDE.** This report predicts −450..−700ms/entry for candidate #1 (§1, §4 "the biggest single win"). CORRECTION: that prediction was priced against the LOAD-INFLATED ~855–1185ms entry task this report's own §1 later flags as unreproducible. On the calm post-deferral entry task (~130–230ms at 4x), the measured marginal saving is ~15–25ms/entry; the real, mechanically-certain wins are per-SWITCH (`upd`'s `offsetWidth` rode every switchTab — measured ~−50..−68ms/drill-switch) and on slower hardware, where a forced full-document reflow of 11.6MB costs proportionally more. Candidate #1 is the SMALLER lever on entry, not the bigger.
>
> **3. DEFERRAL CORRECTNESS.** The cold verify (§3 Confirmed #3) reports 9/9 panes "current after settling" on 45 cycles. CORRECTION: true but incomplete — it checked AFTER the drain. A hidden pane whose `getStats()` is read DURING the drain window returned stale data: `DeepWhiteboard.getStats()` reads render-assigned `_steps`, and progress.js + session-progress.js read `wbEl().getStats()` while the board is hidden (the "do this next" recommendation) — so for ~400ms after a topic change the recommendation could be computed on the PREVIOUS topic's board. `card_identity` (an immediate-check gate) exposed it: flaky 2/3 on the deferral, deterministic-FAIL once the reflow lever shifted timing. Fixed by marking DeepWhiteboard `eagerTopic` (6512f23); drill needed no fix (`publishBanks` reseeds its set synchronously). The deferral's stated invariant ("stale content never on screen") held for the VISIBLE pane; the gap was stale data read from a HIDDEN pane by another component.
>
> ## ⚠ COLD-VERIFY PINS + W1 INTEGRATION (dated 2026-07-19, later in `perf/chunk-proto`)
> A cold verifier re-measured round 2 on a calm box; the branch then integrated master's W0+W1. Three more corrections:
>
> **4. DEFERRAL MAGNITUDE (claim 2) pinned lower.** Round 2's honest report gave ~−40% drill-active input→paint at 4x, explicitly flagged load-noisy. The cold lane's MATCHED-LOAD ratios govern: the deferral is **~−20% @4x** on drill-active input→paint. The proto absolute (~110–125ms) was accurate; the round-2 BASE was load-inflated relative to its own proto, roughly doubling the apparent delta. Direction and the 1x-wash stand.
>
> **5. FORCED-REFLOW RUNTIME (claim 3) conceded to ~0.** Round 2 reported ~−16–28ms/entry and ~−50–68ms/drill-switch at small n under load. The cold lane measured the lever's runtime effect at **~0**, and the mechanism confirms it in source: in `switchTab`, `upd()` (where `void offsetWidth` was) fires BEFORE `swap()` adds `.pane.on`, and `.pane` is `display:none` until `.on` (styles.css:319–320) — so the incoming pane is outside the layout tree when the read fires; the removed reads never forced the reveal layout. The lever stays as harmless hygiene (four genuine layout-forcing reads gone, zero UX, gate-clean); any BOOT-time value (this audit attributed large boot reflow costs — `pinTop`/`scrollTop`/`upd` — to these same reads) is UNMEASURED and therefore UNCLAIMED.
>
> **6. GATE COUNT, and a SECOND hidden-pane class.** (a) Round-2 reports said "42 checks" — the gate is **40** (43 once W1's `flow_data`/`flow_handoff`/`flow_evidence` land on the integrated build). (b) Correction #3's "drill needed no fix" was true for `card_identity` (post-render reads) but INCOMPLETE: W1 integration exposed that `sessStats() → flowRec()` reads `drillEl().getStats().bankTot` while #drill is HIDDEN to place W1's seg recommendation pip + terminal strips, and `flowPip()` fires on the initial `deeptopicchange` before the drain — so a deferred drill left the pip unplaced (VR-proven: pip absent at rest, present the instant flowPip re-fires with drill drained). Fixed by `DeepDrill eagerTopic` (d96e421). `DeepTradeOffs.getDecisions()` (pure topic data, not live state) instead took the registry-direct path (1b2a604) after the cold lane proved that race user-reachable (open Mixed Fire ~90–280ms after a topic change → 2/8 items from the old topic). THE RULE across all three: **EAGER** when the cross-component read is LIVE working-set state render accrues (whiteboard/drill `getStats()`); **REGISTRY-DIRECT** when it is PURE topic data (trade decisions).
