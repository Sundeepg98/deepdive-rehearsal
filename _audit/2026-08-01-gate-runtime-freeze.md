...
(`seg_state`: 31ms connect against a 337ms cold launch, both measured by the cold verify at this
tip; an earlier draft quoted "34ms against 450ms" and neither half reproduced). Off for two
independent reasons, either of which is
...
### The discovery that justifies building a thing you intend to disable

**The first implementation of Phase 2 would have hung the gate, and no amount of review would
have found it.** `chromium.launchServer()` + `chromium.connect()` is a sound-looking design. But a
connected Playwright client holds an open WebSocket; an open socket keeps Node's event loop alive;
and the shim could not let a check's `browser.close()` through, because over `connect()` that
closes the browser for every check queued behind it. Overriding `close()` to drop only the check's
own contexts -- the obvious fix, and the correct one for the server -- left the socket open.

Measured, not reasoned: **`room_browser` completed every assertion and then hung until killed at
100 seconds (exit 124), while `seg_state` exited 0 on the same configuration.** The difference is
that `seg_state` routes its exit through `B.finish()`, which calls `process.exit()` and force-exits
past the live handle. **Three gate checks exit naturally** -- `room_browser`, `topic_contract`,
`cold_open` -- so the gate would have hung on the first one it reached.

Rebuilding on `connectOverCDP` removed the dilemma outright: Playwright does not own a browser it
reached over CDP, so `close()` terminates the connection and leaves the browser up. No override,
no hang. That rewrite is also what made an honest both-ways measurement possible at all -- without
it, the Phase 2 verdict would have been "it hangs", which is a fact about my shim rather than
about sharing.

**This is the wave's clearest argument for running things rather than reasoning about them.** The
2.3% profile number was enough to decide Phase 2's fate on paper. Building it anyway surfaced a
failure mode that paper could not: a design that is correct about the server and fatal to the
client, visible only when a check that exits naturally meets a connection that never closes.
### The part of Phase 2 worth keeping even though Phase 2 is off

**Sharing is refused mechanically, not by a list.** The server publishes the launch terms it was started with, and the
...
**The one disagreement is a check that disagrees with itself.** `touch_floor`, measured alone and
serially on a quiet box: **18/90 = 20.0% [13.0-29.4]**, pooled across this wave's 30 runs and the
cold verify's independently receipted 60. Every geometry failure is the byte-identical
`{"w":42.2,"h":42.2}` -- `44 x 0.96`, which the verify pinned to `panelIn`'s literal `from`
keyframe. The control is not short and the sample is not even mid-flight: **it is taken before the
animation advances at all.** So the honest form of the claim is: **the fast lane is
verdict-identical wherever the checks themselves are deterministic.** It is written that way in
the acceptance document and it should be quoted that way.

**This wave published that rate as 13.3% (4/30) and the cold verify corrected it.** Mine was the
one number in the wave with no receipt in the tree -- hand-entered, in a document that opens by
claiming every number in it is generated. The verifier's 60 runs are receipted, the two samples
are statistically compatible (p = 0.402), and the receipted number wins. That rule is only worth
anything if it applies to the person who wrote it down.

---

## The cold verify

`_audit/2026-08-02-gate-runtime-coldverify.md`, by gr-verifier, independent and with no shared
context -- committed verbatim beside this document.

**No blocking findings.** It re-armed all six mutants (6/6 detected), wrote two novel mutants of
its own in classes this wave did not cover (keyboard-a11y, navigation-routing; 2/2 detected),
proved the serial default untouched **mechanically** rather than by reading -- an AST comparison
of the registry literals at both revisions, native/browser/order identical, 76 checks -- and
re-checked that proof plus the share/cold policy against the moved tip. It re-derived 14 of this
wave's numbers exactly and found three that did not reproduce; all three are corrected in the
acceptance document, and all three corrections ran against this wave's own interest.

Its own finding, which this wave did not have: **`focus_ring` also produced a fast-only red**
(1/7 fast vs 0/52 serial, p = 0.119, instrument negative-controlled). Unproven, recorded, and
structurally important because `focus_ring` sits in the parallel pool rather than the serial tail.
...
| | wall |
|-|-----:|
| serial, quiet box | **602 s** (10.0 min) |
| serial, warm / lightly contended | 692-877 s |
| serial, under a sibling agent's full browser gate | **915 s** (15.2 min) |
| `--fast --jobs 4` | 427-938 s across fourteen measured runs |
| `--changed` on a `src/topics-md` edit | 16 checks instead of 76 |

**The serial gate's own wall time spans 602-915 s on one machine with one tree** -- a 1.52x range
driven entirely by what else the box was doing. That is worth more than any single speedup
figure, because it is the reason a speedup figure cannot be a constant, and the reason the
acceptance battery measures VERDICTS rather than seconds.
...
1. **Fix `touch_floor`'s sampling. RULED (team-lead, 2026-08-01) as a separate CHECK-HYGIENE
   ...
   entry RETIRES and the standing order is amended. The cold verify sharpened the target: the
   guard must gate on **transform is identity**, not on two agreeing reads, because 42.2 is
   `panelIn`'s FIRST keyframe and agreement is easiest before the animation starts. Its battery
   should also cover the second failure class the verify found (browser death, 2/60), which no
   at-rest guard addresses. Removing a **~20%** false red from a 76-check gate
...
4c. **Give `focus_ring` the 30-run treatment under `--fast` BEFORE anyone promotes the fast lane.**
   Raised by the cold verify: it went red once under `--fast --jobs 4` against 0/52 across serial
   contexts (p = 0.119, not significant, instrument negative-controlled at 50 solo runs). One
   event cannot separate load-sensitivity from rare nondeterminism -- but `focus_ring` is in the
   PARALLEL POOL, so if it is the former, the serial tail is missing a member. This is the highest-
   value open question the wave leaves behind, because it is the only one that could change the
   shipped lane's composition.
4d. **`--fast` stamps `certifying: true` into its `--verdicts` file** while its own summary line
   prints `[parallel -- not the capture of record]` and both audit documents say the fast lane is
   not certified for trains, captures or CI. The field tracks COVERAGE (`not CHANGED`), which is
   defensible in isolation and misleading in name: a consumer reading `certifying` would read a
   `--fast` run as a certification -- the exact "partial run read as a full one" failure the
   `--changed` banner exists to prevent. Raised by the cold verify, NOT fixed here: it is a
   behaviour change to the runner arriving after the verify signed off on that runner, and it
   deserves its own small change rather than a post-verdict edit. Recommend renaming the field to
   `full_coverage` and adding a separate `capture_of_record` that only a no-flag run sets.
...
| `_audit/2026-08-02-gate-runtime-coldverify.md` | the independent cold verify, verbatim, by gr-verifier |
...
The capture above was taken while another agent saturated the machine. With the box genuinely
clear -- gr-verifier finished, zero Playwright processes, nothing else running -- the same serial
gate on the same commit:

**76 PASS, 0 FAIL, 0 SKIP, exit 0, in 602.2 s (10.0 min).**

Same tree, same code, same 76 verdicts; **915 s contended against 602 s clear.** The pair is the
useful artifact: it shows the gate is green in both conditions, and it prices contention at about
+52% wall time without moving a single verdict. Anyone reading a wall-time number out of this wave
should read that range first.
...
================================================================
  ...
  syntax_check              PASS  SYNTAX CHECK: PASS  (614 modules parse; 52 aggregator files skipped)
  global_collisions         PASS  GLOBAL COLLISIONS: PASS  (690 top-level globals, each declared in exactly one module)
  build_integrity           PASS  BUILD INTEGRITY: PASS  (12249669 bytes, 0 unresolved, 9 panes + 7 overlays, build SYNCED the deliverable, HEAD-match DEFERRED -- 4 uncommitted path(s) [_audit/2026-08-01-gate-runtime-acceptance.md, _audit/2026-08-01-gate-runtime-freeze.md, test/check_all.py]: commit src/ AND the rebuilt deliverable together, or CI will reject the pair)
  css_syntax                PASS    PASS All checks passed (18 keyframes validated)
  room_static               PASS  ROOM STATIC: PASS  (codemod=0, styles.css infinite=0, 6 room blocks + rebind, boot stamped)
  room_contrast             PASS  ROOM CONTRAST: PASS  (12 rooms: ink/bg >= 4.5, on-slab/solid >= 5.0)
  slab_ink                  PASS  SLAB INK: PASS
