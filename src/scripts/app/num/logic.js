...
.num-h{font:var(--font-weight-heavy) 10px var(--sans);letter-spacing:var(--track-px-0-6);text-transform:uppercase;color:var(--mut2);margin-bottom:var(--space-12);display:flex;align-items:center;justify-content:space-between;gap:var(--space-10)}
.num-reset{font:var(--font-weight-semibold) 10px var(--sans);color:var(--mut);background:transparent;border:1px solid var(--bd);border-radius:6px;padding:var(--space-3) var(--space-9);cursor:pointer;text-transform:none;letter-spacing:var(--track-px-0-2);transition:color var(--duration-fast) var(--ease-base),border-color var(--duration-fast) var(--ease-base)}
.num-reset:hover{color:var(--acc);border-color:var(--acc)}
.ninp{display:grid;grid-template-columns:1fr 1fr;gap:var(--space-12)}
.ninp label{display:flex;flex-direction:column;gap:var(--space-6);font:var(--font-weight-bold) 11px var(--sans);color:var(--mut);letter-spacing:var(--track-px-0-2)}
