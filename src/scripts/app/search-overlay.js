...
(function () {
  ...
  /* B7: on a miss, offer the nearest topic/view names by edit distance -- typo tolerance
     ("kafak" -> "Kafka"). A cheap Levenshtein against each label and its words; keep the
     few within a length-scaled threshold. */
  function _lev(a, b) {
  ...
  function closestMatches(q) {
    q = (q || '').toLowerCase().trim(); if (q.length < 2) return [];
    var thresh = Math.max(2, Math.floor(q.length / 3)), out = [];
    function consider(label, kind, id) {
      var lc = label.toLowerCase(), best = _lev(q, lc), words = lc.split(/\s+/);
      for (var w = 0; w < words.length; w++) { var d = _lev(q, words[w]); if (d < best) best = d; }
      if (best <= thresh) out.push({ d: best, label: label, kind: kind, id: id });
    }
    for (var i = 0; i < TOPIC_INDEX.length; i++) consider(TOPIC_INDEX[i].label, 'topic', TOPIC_INDEX[i].id);
    for (var m = 0; m < MODULES.length; m++) consider(MODULES[m].label, 'view', MODULES[m].id);
      ...
      if (inputEl.value.trim()) {
        ...
        if (near.length) {
          ...
          near.forEach(function (nm) {
            ...
            sug.addEventListener('click', function () { navigateTo({ kind: nm.kind, id: nm.id }); });
