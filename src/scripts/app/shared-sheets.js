...
.opt-w .pw{font-weight:800;color:var(--mut2);text-transform:uppercase;font-size:9.5px;letter-spacing:.5px;margin-right:7px;text-shadow:0 0 6px rgba(83,74,183,.06)}
.opt-w b{color:var(--accink);font-weight:700;text-shadow:0 0 8px rgba(83,74,183,.08)}
`);

var ANS_SHEET = new CSSStyleSheet();
ANS_SHEET.replaceSync(`
.qq{font-size:15.5px;font-weight:680;background:linear-gradient(90deg,var(--ink),var(--accink));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.45;letter-spacing:-.01em}
...
.judge .hint{font-size:9px;font-weight:700;background:linear-gradient(90deg,var(--acc),var(--acc2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
.mock-body{padding:19px 18px 22px;background:linear-gradient(180deg,transparent 0%,rgba(83,74,183,.01) 100%)}
`);
...
.mbeat b{color:var(--accink);text-shadow:0 0 8px rgba(83,74,183,.08)}
`);
