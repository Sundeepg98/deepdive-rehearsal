    ...
    function build() {
      ...
      if (buckets && buckets.length) {
        menu.innerHTML = buckets.map(function (b) {
          return '<div class="tn-group" role="presentation"><span class="tn-g-dot" style="background:var(--room-' + b.group.id + ')"></span>' + b.group.label + '</div>' + '<div role="group" aria-label="' + b.group.label + '">' + b.ids.map(tnItem).join('') + '</div>';
        }).join('');
      } else {
