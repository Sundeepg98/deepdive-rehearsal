...
(function () {
  ...
  function createElements() {
    ...
    overlayEl.style.cssText = 'position:fixed;inset:0;z-index:var(--z-modal);background:rgba(0,0,0,.55);opacity:0;transition:opacity var(--duration-slowest) var(--ease-base);pointer-events:none';
    ...
    tooltipEl.style.cssText = 'position:fixed;z-index:var(--z-modal-top);width:' + TOOLTIP_WIDTH + 'px;max-width:90vw;background:var(--card);border:1px solid var(--bd);border-radius:20px;padding:var(--space-24);box-shadow:0 20px 60px -20px rgba(0,0,0,.4);opacity:0;transform:translateY(20px) scale(.96);transition:all var(--duration-slowest) var(--ease-glide);pointer-events:auto';
    ...
    tooltipEl.innerHTML =
      ...
      '<div style="display:flex;align-items:center;justify-content:space-between">' +
        ...
        '<div style="display:flex;gap:var(--space-8)">' +
          (index > 0 ? '<button type="button" id="_tour-prev" style="background:var(--card);border:1px solid var(--bd);border-radius:10px;padding:var(--space-6) var(--space-14);font:var(--font-weight-semibold) 12px var(--sans);color:var(--mut);cursor:pointer">Prev</button>' : '') +
          '<button type="button" id="_tour-next" style="background:linear-gradient(135deg,var(--acc) 0%,var(--acc2) 100%);border:none;border-radius:10px;padding:var(--space-6) var(--space-18);font:var(--font-weight-semibold) 12px var(--sans);color:#fff;cursor:pointer;box-shadow:0 2px 8px var(--acc-a30)">' + (index < total - 1 ? 'Next' : 'Finish') + '</button>' +
        '</div>' +
      '</div>' +
