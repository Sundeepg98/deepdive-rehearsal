...
Int: You said serve stale. The key just expired --- Redis deleted it. What stale value are you serving?
None, and that's the detail that decides whether "serve stale" is a design or a slogan. If I let the store's own TTL be my expiry, then **expiry and absence are the same event** and there is nothing left to serve --- the first request after expiry is a hard miss and the herd forms exactly as before. So stale-while-revalidate needs **two expiries**: a physical TTL on the entry that is deliberately much longer, and a **logical freshness timestamp stored inside the value**. A read compares against the logical one --- fresh, serve it; stale but present, serve it immediately and kick off exactly one background refresh; genuinely absent, and only now do I have to block, which is where single-flight earns its keep. That also turns my three "layers" into one design rather than a menu of alternatives: probabilistic early refresh keeps you in the first case, stale-serving covers the second, single-flight covers the third, and the physical TTL demotes to being a garbage collector rather than a correctness mechanism. The trap is claiming stale-while-revalidate while `EXPIRE` is still the only expiry you have.
...
Int: Pre-warm the hot keys --- which ones? You just flushed the thing that knew.
That's the hole, and if I haven't prepared for it in advance I cannot warm at all, only ramp. The hot set has to be recorded **somewhere that is not the cache**: sample it continuously in normal operation --- a periodic dump of the top keys by access, or derive it from the request log --- so the warm list is a standing artifact rather than something I try to reconstruct during an incident. Redis will tell you (`--hotkeys`, or `OBJECT FREQ` under LFU) but only while it still holds the data, which is precisely what I destroyed. With nothing recorded, the honest play is not to guess: **ramp instead of warm**, admitting a fraction of the read rate and raising it as the hit ratio climbs, with single-flight on so each key rebuilds once, using the database's own load as the control signal. And the real lesson sits one step earlier --- the flush should not have been reachable. **Versioning the namespace** gets me the same clean slate by bumping a key prefix, while the old keyspace stays intact and expires on its own: no cliff, and the previous data is still there if I need to roll the version back.
...
Int: Your filter says an id definitively does not exist. I created that product one second ago. You are now returning a false 404 on real data --- fix it.
That's the operational cost of the filter, and it is exactly where they break in production. The "no false negatives" guarantee is about **the set the filter actually contains** --- it says nothing about that set being current, so an id created after the last rebuild is genuinely absent and I confidently reject a real product. Two fixes, different in kind. First, **make insertion part of the write path rather than the rebuild**: a standard Bloom filter supports adds, so the create transaction --- or the CDC stream behind it --- sets the bits, and the staleness window collapses from the rebuild interval to replication lag. Second, **fail open on the boundary, never closed**: if the filter is unavailable, or I know it may be behind, I skip it and pay for one ordinary lookup, because the cost of the filter being wrong is a lost sale and the cost of skipping it is a single query. And the thing that catches people out later --- a plain Bloom filter **cannot delete**, so removals need a periodic rebuild or a counting variant, which is the other reason the rebuild schedule is not something you get to forget about.
...
Int: The L1 is in-process. Your invalidation deletes from Redis. What deletes it from the fifty L1s?
Nothing does --- and that is the honest cost of the L1, which I should name rather than let someone discover. Deleting from Redis reaches the shared tier only; every app instance keeps serving its own copy until that copy's own TTL runs out, so my worst-case staleness is now the **L1 TTL**, and worse, **fifty instances can disagree with each other at the same moment**. That is a nastier failure than one stale value, because a user refreshing the page flaps between old and new depending on which instance they land on. Three ways to live with it. Keep the **L1 TTL very short** --- single-digit seconds --- so the window is bounded and quantified, which for a celebrity profile is genuinely fine and is what I'd reach for first. If seconds are too long, add a **pub/sub invalidation broadcast** so the write publishes a key-deleted message and every instance drops its copy --- but that is best-effort delivery, so the TTL still has to backstop it. And be deliberate about **what is even eligible** for the L1: it is the right structure for a hot, read-mostly, tolerant value, and the wrong one for anything a user must see their own write reflected in.
...
Int: So you switch to LFU. A product goes viral this morning. Why might it never make it into the cache at all?
Because LFU ranks on **accumulated** frequency, so a brand-new key starts at a count of one and has to out-compete keys that have been building counts for weeks. The incumbents are protected precisely because they *used* to be popular, and a genuinely hot newcomer can be evicted over and over before it ever accumulates the history to survive --- that's cache pollution running in the opposite direction from the scan, and it is why naive LFU is rarely what anyone actually wants. The saving grace is that real implementations aren't naive: Redis's `allkeys-lfu` **decays** the counter on a timer (`lfu-decay-time`), so yesterday's popularity fades and today's can displace it, and the counter itself is probabilistic and logarithmic so it saturates instead of growing without bound. Tuning those two knobs is the actual work, and getting the decay wrong reproduces exactly the failure you just described. The framing I'd hold onto is that **the two policies fail on opposite inputs** --- LRU is destroyed by a scan, LFU by a shift in what's popular --- so the choice follows from which of those your workload really does, and a workload that does both wants a segmented or adaptive policy rather than a side.
...
Int: Deletes fix the two writers. Now a reader that missed *before* the write finishes populating the cache *after* your delete. Does delete-don't-update save you there?
No --- and that is the honest limit of it. I closed the write-write race and the **read-write** race is still wide open. The sequence: a reader misses, reads `v1` from the source, and stalls; the writer commits `v2` and deletes the key; the stalled reader then populates the cache with the `v1` it is still holding in memory. The cache serves `v1` against a source holding `v2`, permanently, and once again nobody errored and nothing detects it. It is rarer than the write-write case, because it needs the reader's stall to straddle the whole write, but it is the same shape and it absolutely happens under load. There is no cheap complete fix, which is exactly **why the TTL is not optional even with perfect invalidation logic** --- it is the only mechanism that recovers from a race you did not close. If I need the window tighter: **delay a second delete** a few hundred milliseconds after the write (the double-delete --- it shrinks the window, it does not close it), populate with a **conditional write** so a stale reader's `SET` loses a version check, or move invalidation onto **CDC** so it is driven by commit order rather than by application timing. The senior move is naming which race each mechanism closes and admitting which one the TTL is quietly carrying.
...
Int: Replay recent invalidations after a failover. How far back is "recent", and who tells you the failover happened?
Both halves need a real answer or that mitigation is a sentence, not a plan. On the **trigger**: the client has to detect the topology change itself rather than wait for a human at 3am --- Sentinel and Cluster both publish a failover event, and the reconnect is itself the signal, so the replay hangs off the client's own reconnect handler and fires automatically. On the **window**: it is bounded by the replication lag at the instant the primary died, which is a number nobody can recover after the fact --- so I do not try to compute it. I pick a conservative constant comfortably above the observed p99 lag, seconds rather than minutes, and replay everything inside it. That works because the costs are **wildly asymmetric**: replaying an invalidation that was not needed costs one extra cache miss, and skipping one that was needed costs permanent staleness on a key nobody will ever notice. And the framing I want to land is that this **narrows the window, it does not close it** --- I cannot know what the dead primary acknowledged and never shipped, so the TTL is still the backstop underneath. It is also the sharpest argument for CDC-driven invalidation: the log survives the failover independently of my process, so I am replaying a durable record of what actually *changed* rather than my own memory of what I *told* the cache.
