...
Follow: The calls that tripped this breaker were charges that *timed out*. When the queue drains, what exactly are you re-sending?
Something whose outcome I do not know --- and that is the part this design quietly gets wrong. A timeout is not a failure, it is an **unknown**: the charge may have reached the processor, been authorized and committed, with only the *response* lost on the way back. So the intent I durably enqueued may be a charge that has **already happened** --- the fallback fires on the *timeout* as well as on the open breaker, so the ambiguous calls and the never-sent ones sit in the same queue, indistinguishable --- and draining the queue re-executes it, the fallback converting a dependency outage into a double charge, which is a *correctness* incident rather than the availability one I was managing. Worse, it is quieter: the outage got a page, and this gets a customer email days later. The fix is not in the breaker, it is in the operation --- the enqueued intent carries a **client-generated idempotency key**, minted once per intent and reused on every attempt, so the drain re-sends the *same* operation rather than a second one and the processor replies with the original outcome instead of charging again. If the dependency offers no such key then the write was never deferrable and this was the wrong branch: you **reconcile** instead --- hold the order PENDING and query the processor for its state before deciding. The rule that generalizes past payments: a breaker may only queue a write it can prove is safe to repeat.

Senior: Knowing the fallback is **the least-tested code you own** --- it runs only when something has already failed --- and that you therefore have to exercise it on purpose (force the breaker open, game days, dark launch), is the operational maturity that separates designed degradation from aspirational degradation. The sharper half is that a queued *write* fallback is only legal if the operation is idempotent: a timed-out call is an **unknown**, not a failure, so a drain without an idempotency key is a double-charge generator.
...
**The timeout** --- and I'd argue it isn't a prerequisite so much as the thing doing most of the work, with the breaker as an optimisation on top. Without a timeout a call can block indefinitely, so it never produces an outcome; and a breaker is a state machine driven by outcomes, so it records nothing, its error rate stays at zero, and it never trips. That is the failure that catches people out: the breaker is present, configured, green on the dashboard, and structurally incapable of firing, because the thing it counts never happens. **The timeout is what converts a hang into a countable failure.** It is also the only mechanism that bounds the damage of the *first* wave of calls, before any breaker has enough samples to decide anything --- the breaker needs a minimum volume over a window, which means by construction it is late, and the timeout is what protects you during that gap. So I'd build it first, and I'd set it deliberately rather than by habit: derived from the dependency's own p99 plus headroom, not a round number, and always shorter than the caller's own deadline, or I've built a system where my caller gives up before I do and I'm holding a thread for a response nobody will read. The one-line version: **a breaker on a system with no timeouts protects nothing** --- and if I could only have one of the two, I would keep the timeout.
...
**A breaker on a low-traffic dependency does not heal on its own, because nothing is asking it to.** The transition is evaluated when a call arrives, so the breaker recovers on the first call *after* the cooldown, not at the cooldown --- and if no call arrives for an hour, it sits open for an hour, long after the dependency came back. On a busy path the distinction is invisible: traffic is continuous, so "after the cooldown" and "at the cooldown" are milliseconds apart and nobody ever notices. On a rare path it is genuinely surprising, and it bites hardest exactly where it hurts --- the nightly batch job, the monthly billing run, the admin action somebody takes twice a week. The first call after the outage pays a failure that the breaker could have absorbed hours earlier, and worse, that call is often the one a human is watching. Two ways out, and I'd pick by how much the path matters. **Accept it and shorten the cooldown** for low-traffic dependencies, since the cost of an occasional probe is trivial when traffic is trivial. Or **make recovery active**: a background health probe that drives the state machine independently of caller traffic, which is what you want when the rare path is also an important one. The general point is that **lazy evaluation is an implementation detail that leaks into behaviour**, and it leaks in inverse proportion to traffic --- so a breaker's semantics are not actually the same on your busiest and quietest dependencies, even with identical configuration.
...
Model: **A shared dependency failing is where a local problem becomes a company-wide one, and it does it along two paths that are worth separating.** Without breakers, every caller **retries**, so the offered load on an already-struggling dependency multiplies at exactly the wrong moment --- that's the storm, and it's what stops it recovering. Meanwhile every upstream service sitting on a call to it **holds a thread waiting for a timeout**, so pools drain and those services start failing requests that had nothing to do with the dependency at all --- that's the cascade, and it's how a recommendations outage takes down checkout. The two are independent and both are fatal. **A per-dependency breaker addresses both**: it fails fast, so the retries stop reaching the dependency and it gets room to recover, and it releases threads immediately instead of parking them on a timeout, so the caller's pool stays healthy. Per-dependency matters --- one global breaker would let a failing recommendations call open the path to payments. Then **fallbacks** decide what the user sees: a cached list, an empty rail, a degraded-but-alive page. The result is that the failure stays confined to the features that genuinely need that dependency, instead of propagating along every thread pool that touched it.
Int: Your breaker protects the caller. From the failing dependency's point of view, is a breaker actually good news?
Mostly yes, and the *mostly* is the interesting part. The good news is real and is often stated backwards: a breaker is the **only** thing that reliably stops a struggling dependency being retried into the ground, and a service in overload usually recovers the moment offered load drops below capacity --- so hundreds of callers failing fast is precisely the breathing room it needs. Compared to the alternative, where every caller retries and the dependency never gets a quiet second, that is a gift. But there are two ways it hurts, and I'd want the dependency's owner to hear them. **Recovery is invisible to it**: once breakers are open, traffic goes to zero, so the dependency looks perfectly healthy on its own dashboards while nobody can use it --- and its owners can be staring at green graphs during an outage that is entirely about them. And **the return is synchronised**: many independent breakers with the same cooldown all close at roughly the same moment and hand it full production load in one step, which is the recovery herd, and it can knock over a service that had genuinely recovered. So the neighbourly version of a breaker is one that **jitters its cooldown**, **ramps rather than resuming at full rate**, and whose state is **visible to the dependency's team** --- because "all my callers have given up on me" is the single most useful signal that service could have, and it is the one thing its own metrics structurally cannot tell it.
...
**A bulkhead on the recommendations call** --- a hard cap on how much of the thread pool that one dependency may ever consume, say twenty of two hundred threads. And the reason I'd pick it over the breaker, if I only get one change, is about **timing**. A breaker is a statistical device: it needs a minimum number of calls over a rolling window before it will trip, so by construction it is *late*, and the cascade here happened in seconds --- the pool was gone before any breaker had a quorum of samples to reason about. A bulkhead needs no samples and no decision. It is a static limit that is already in force during the very first hung call, so the worst case is that recommendations occupies its twenty threads and the remaining one hundred and eighty keep serving checkout. The site degrades instead of falling over, which is the entire objective. The deeper reason it is the right answer to *this* incident is that the root cause was never recommendations being unhealthy --- it was that **a non-critical feature was able to consume a resource the critical path depends on**, and the bulkhead is the only mechanism that addresses that directly rather than reacting to it. I'd still want the breaker, because a bulkhead alone means those twenty threads stay pinned for the whole outage and the feature never fails fast --- but bulkheads bound the blast radius and breakers shorten its duration, and if the site is down in four seconds, the bound is what I need.
...
Model: **What you've built is a recovery thundering herd, and the breaker is the thing causing it** -- many instances (or requests) go half-open and slam back to full traffic simultaneously the instant the dependency recovers, re-overloading a still-fragile service and knocking it down again, which re-opens the breakers, producing a flapping recovery loop. The fix mirrors retry jitter: keep the half-open probe small (only a few calls through, rest still fast-failed), jitter the cooldown so instances don't all probe at the same instant, and ramp traffic up gradually after closing rather than resuming 100% at once -- so testing recovery and resuming traffic don't themselves re-break the dependency.
Int: Small probes, jittered cooldowns, ramped admission -- that is three tuning knobs on every instance and you have two hundred instances. Is this the right layer to be solving it at?
No, and I think that is a more useful answer than a fourth knob. Every one of those mechanisms is **local**: each instance judges the dependency's health from its own small sample and ramps its own traffic on its own schedule. But recovery capacity is a **global** quantity -- the dependency has one number, and what it actually receives is the sum of what two hundred independent controllers decide to send. No instance can observe that sum, so each is tuning against a variable it cannot see, and the fleet's behaviour is emergent rather than designed. That is why you keep needing another knob: you are approximating a global control loop with local heuristics, and it will keep almost working. The category error underneath is what a breaker is *for*. A circuit breaker protects the **caller** from a slow dependency -- it stops you burning threads and latency on calls that are going to fail anyway. It was never designed to protect the **dependency from its callers**, and this incident is what using it that way looks like. The mechanism that belongs here lives on the other side: a **concurrency limit or a bounded queue on the dependency's own inbound path**, so it accepts what it can serve and rejects the rest immediately. That is admission control exercised by the only party that knows the capacity, it needs no coordination between callers, and it behaves correctly whether there are two callers or two thousand. Keep the breakers, because they are still doing their real job. Just stop asking them to be the recovery mechanism. And if the dependency is not yours to change -- a third party, another org -- then the honest second best is to move the decision from per-instance to **shared**: a mesh sidecar policy or a gateway that sees the whole fleet's traffic as one number and can ramp it as one number.
...
**By dashboarding breaker state against the dependency's *own* health and alerting on the disagreement** --- because that comparison is the single metric that separates "my breaker is wrong" from "my dependency is broken," and neither signal can tell you that alone. A breaker transitioning twelve times an hour while the dependency's error rate, measured **at the dependency or by its other callers**, sits flat at 0.5% is a mistuned breaker by definition: the dependency is not doing anything a breaker should react to. Looking at either graph on its own is exactly how this ships --- my breaker metrics look dramatic and justified, and the dependency's team sees nothing wrong at all, and both parties are reading their own instrument correctly. Two things I'd add around it. **Alert on the transition *rate*, not on the state**, because an open breaker is a legitimate event and a breaker that opens and closes repeatedly never is --- flapping is a distinct signal with a distinct meaning, and it deserves its own alarm rather than being buried in the noise of ordinary opens. And **run a new breaker in shadow first**: evaluate the state machine and record what it *would* have done without actually failing any calls, for a week, against real traffic. That is the cheapest possible way to discover the threshold is under your noise floor, and it costs users nothing. The framing I'd land: a breaker is a **control system tuned against a signal**, so the tuning has to be validated against the signal's actual distribution --- and a threshold chosen from a round number rather than from the observed error distribution is a guess, whatever the number is.
