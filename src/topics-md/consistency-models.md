...
If you modeled it as a **register** and resolve with last-write-wins, one of the increments is **silently lost**: both replicas read 5, both compute 6, both write 6, LWW picks one --- the count is 6 when it should be 7, with no error and no conflict surfaced. That is the classic lost update, and it means my own "eventual is fine for a like count" was only half an answer. Eventual consistency does not make a **read-modify-write** safe --- it turns one into a lost-update generator. The fix is to stop storing the *value* and start storing the *operation*: a **G-counter CRDT** (grow-only), where each replica increments only its own slot and the value is the sum of all slots (merge = element-wise max), so two concurrent increments on different replicas both survive and merge to 7 --- deterministically, with no coordination. Name the limit before they ask: a G-counter cannot go down, so the moment you support **un-like** you need a **PN-counter** --- a *pair* of those maps, P and N, each merged the same way, value = sum(P) - sum(N). Or, if you would rather not run a CRDT, make the increment an atomic operation against a single owner for that key (a Redis `INCR`, a DynamoDB `ADD`). So: eventual is fine for the like count, *provided the merge is right*.
...
Senior: Knowing the ceiling is **per consensus group and cannot be raised by adding replicas** (adding them makes it slower; you scale by adding *shards* --- which is why Spanner runs **multi-Paxos per tablet** and CockroachDB runs **Raft per range**, rather than one global group) --- and pricing the cross-region write concretely (~75-90 ms to the second-closest region, so 100-200 ms per call) leading to **localize authority** --- is the Staff answer here.
...
Senior: Answering the "you're just labeling things" challenge by proving **each label changed a route, a type, or a failure behavior** (leader ReadIndex at ~2 ms and refuse-under-partition; CL=ONE with a G-counter) --- and answering "assume no partitions" with **the design barely changes, because ELC is where the cost lives** --- is the strongest available close on this topic, and it lands the whole PACELC point without reciting it.
...
The trap this closes is the innocent-looking one from earlier: *"eventual is fine for a like count."* It is --- **but only if the merge is right.** Modeled as a register with LWW, two concurrent likes both read 5, both write 6, and one increment is **silently lost**. Eventual consistency does not make a read-modify-write safe; it turns one into a lost-update generator. Model it as an *operation* --- a **G-counter**, where each replica increments its own slot and the value is the sum --- and both survive. It is grow-only by construction, so the moment you support **un-like** you need a **PN-counter**: a *pair* of these maps, P and N, each merged exactly the same way, with value = sum(P) - sum(N).

```js
// A G-counter merges by taking the element-wise MAX of each replica's own slot,
// so two concurrent increments on different replicas both survive.
// (A PN-counter is two of these -- P and N -- with value = sum(P) - sum(N).)
const merge = (a, b) => {
  const out = {};
...
- The fix | sub | So I model it as an operation, not a register: a G-counter, where each replica increments its own slot and the value is the sum, so both increments survive and merge deterministically with no coordination. It's grow-only, so once un-likes exist it's a PN-counter --- two of those maps, value = sum(P) minus sum(N).
...
- CRDTs | sub | Or design the data so concurrent updates merge commutatively with no coordination at all --- a G-counter for counts that only rise, a PN-counter where they can also fall, an OR-set for sets, a sequence CRDT for collaborative text. Both writes survive, deterministically.
...
Model it as an *operation*, not a value: a **G-counter CRDT** (each replica increments its own slot; the value is the sum, merged by element-wise max) so both increments survive --- and a **PN-counter**, which is two such maps with value = sum(P) - sum(N), once the count can also go down. Or an atomic increment against a single owner for that key (`INCR`, DynamoDB `ADD`).
...
Model: **I'd start by naming which guarantee I'm actually buying, because it is a much smaller one than "consistency" and that is the whole point.** Replica reads lag; that is normal and mostly harmless. What users genuinely notice is **their own write vanishing** --- they post a comment, the page reloads off a replica, and it isn't there. So I want **read-your-writes**, a session guarantee, not global strong consistency. Two ways to get it. **Sticky-to-primary**: after a user writes, route their reads to the primary for a short window --- simplest thing that works, and it costs primary read capacity. Or **read from a caught-up replica**: capture the write's log-sequence number and route the read only to a replica that has replayed at least that position --- more machinery, and it keeps the read load off the primary, which matters at scale. Either way the discipline is in the **scoping**: stick the user's *session* and the *affected data*, not everything; keep the window **short but comfortably longer than typical replication lag**, a few seconds; and let every read that doesn't need the guarantee stay eventual. That's the trade I'd defend --- pay narrowly for the one anomaly people actually perceive, and don't buy global coordination to fix a user-experience bug.
Int: You capture the write's log-sequence number and route the read to a replica that's caught up. Where does that LSN actually live between the write response and the next read?
That's the question that decides whether this design is buildable, and there are only a few real answers, each failing differently. **In the client** --- returned on the write response, echoed on subsequent reads. It's the simplest, it works across services, and the catch is that it is now **client-controlled**, so it has to be treated as a hint and **clamped**: an unbounded value would otherwise pin every read to a replica that will never be that caught up, or fall through to the primary, which is a cheap way for a client to accidentally take down my read scaling. **In a server-side session store** --- not forgeable, at the cost of a session lookup on every read, and it doesn't follow the user to a second device or a second tab that never saw the write. **In a sticky cookie**, which is honestly the client option wearing a disguise. And then the case that breaks all three: the read arrives at a **different service** that never saw the write at all, which is why in a real system the token has to travel in the **request context and be propagated through the call chain** --- the same plumbing as a trace id, and about as easy to forget on one hop. When none of that infrastructure exists, that is exactly the moment **sticky-to-primary is the right call** rather than the naive one: it is cruder and costs primary reads, but it needs no token to survive anything. The honest summary is that the LSN approach is better on paper and its entire difficulty is **transporting a causality token through a system that was never designed to carry one**.
...
Model: **Split by operation --- because "what consistency does this system need" is the wrong question, and asking it per operation is the whole design.** The balance and payment path needs strong consistency (linearizable reads, serializable transactions) -- a stale or lost write is a correctness bug -- so pay the coordination latency and choose CP under a partition (refuse rather than serve stale); localize it to reduce cross-region round-trips (keep an account's authoritative copy in one region, or use a Spanner-style TrueTime system if you truly need global strict serializability). The feed/like-count path is eventually consistent -- brief staleness is harmless -- so it is fast, available, AP under partition, and uses a CRDT counter so concurrent increments merge without conflict. A user's own reads get read-your-writes (pin to the primary briefly after a write). Tune it per operation (QUORUM vs ONE, strongly-consistent read flags) rather than one global level.
Int: An account's authority lives in one region. Now a US user pays an EU user. Which region owns that transaction?
Neither, on its own --- and this is precisely where localizing authority stops being free. I optimized the single-account case, and a transfer is inherently a **two-authority operation**, so I have to pick a mechanism rather than pretend the locality survived. **Two-phase commit** across the two regions gives me atomicity and costs a cross-continent round trip **while holding locks on both accounts** --- which is the part that doesn't scale, because a slow or partitioned participant now blocks two balances and a coordinator crash strands them in doubt. **A saga**: debit locally, emit an event, credit remotely, compensate on failure. No cross-region locks, and the price is a window in which the money is provably in **neither** account --- so the design has to make that window **visible and reconcilable** rather than hide it, which is also why the ledger should model a transfer as an entity with a state rather than as two independent balance updates. Or **relocate authority** and co-locate genuinely frequent counterparties, which only works when the pairing is stable. I'd take the saga with an explicit in-flight state, because 2PC on a payment path builds a system whose throughput is capped by the speed of light and whose availability is the *product* of two regions'. And the thing that actually decides it is **how common the cross-region case is**: if it's rare, the saga's complexity stays confined to a rare path; if it's the main use case, then partitioning authority by account was the wrong call in the first place and I should be partitioning by something the traffic respects --- a currency zone, or a corridor.
