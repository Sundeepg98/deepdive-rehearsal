...
Model: **The dashboards are green because they are computed from the table that only contains the records that worked --- this is survivorship bias, and the metric is telling the truth about a population it selected.** The pipeline stores successfully-processed records; failures are logged and discarded; so "100% processed" means "100% of what made it in," which is a tautology, and no amount of staring at that dashboard will ever show a problem. I'd confirm it in one query rather than by argument: compare **log error volume against stored failure count**, and the gap is the loss, with a number attached. The fix has two halves and only one of them is the parser. Wrap the parse and ingest step so a failure **writes the raw payload plus the error to a dead-letter table** --- failures become rows, and rows are queryable --- then expose **"X of Y processed"** rather than a success percentage, because a ratio with a denominator cannot hide a shrinking numerator. Add an alert on **dead-letter volume and NULL rate** so the next instance of this class pages me instead of waiting for a customer to notice. And **back-fill** the dead-lettered records once the parser is fixed, which is only possible because I kept the raw payload. The durable output here isn't the parser fix, it's that **failure became a thing the system can be asked about**.
Int: You've made future loss visible. The customer is asking what they lost over the last eight months. What do you tell them?
The honest answer first: probably that I cannot fully reconstruct it, because that is what silent loss means --- the records were discarded, not stored badly. But "gone" and "unknowable" are different, and I'd work the difference hard before conceding it. The data may be gone while the **evidence** survives: **log retention** may still cover part of the window, the **upstream source** may still hold the originals, and if there is any **raw landing zone** before the parse step, the whole period is replayable through the fixed parser. That is exactly why an immutable raw copy ahead of a fragile step earns its storage --- it makes this question answerable instead of terminal, and I'd name it as the design lesson rather than let it pass. If nothing upstream survives, then what I owe them is a **bound rather than a shrug**: from log error volume over time I can say approximately how many records were lost, when the loss started, and which types were affected --- which is a far more useful answer than "we don't know" and lets them decide what it costs *them*. I'd give them the **shape** of the loss, not just a number, because they can often reconcile against their own side once they know what to look for. What I would not do is quietly present the recoverable subset as if it were the whole story.
Int2: You now write every failure to a dead-letter table. What's the failure mode of *that*?
It becomes a **landfill** --- the same invisibility with extra steps, and I've seen this more often than the original bug. The table fills up, nothing pages on it, and a year later it holds four million rows nobody has ever queried: the loss is now technically visible and practically as hidden as it was before, except everyone believes it is handled. So a dead-letter table is only real with three things attached. An **alert on the rate, not on existence** --- a non-zero dead-letter count is normal, a *rising* one is the signal, and getting that wrong is how people end up muting it. A named **owner and a triage cadence**, because an alert with no owner is a notification. And **bounded retention with a redrive path**, so entries are either reprocessed or expire deliberately, rather than accumulating into something too big to ever look at. The subtler failure mode is worth naming too: **the dead-letter write can itself fail.** If the payload that broke the parser also breaks the insert --- it's oversized, or it has an encoding the column rejects --- then I lose precisely the records I most needed to keep, silently, again, and the whole mechanism was theatre. So that write path has to be maximally dumb: raw bytes into a text or blob column, no parsing, no schema validation, no cleverness --- and its own failure has to be loud.
...
Int: This runbook is excellent once somebody opens it. The whole bug class is silent --- what causes anyone to open it?
That's the gap, and it is the actual reason this class recurs: **the runbook is a response, and the class has no signal**, so in practice it gets opened by a customer complaint --- which is the outcome the runbook exists to prevent. So it needs a companion that is not a runbook, and detection for silent loss has to be built on **conservation rather than on errors**: count what went in and what came out, **per stage**, and alert on the gap. A discrepancy is detectable even when every single component is reporting success, which is exactly the situation here. Next to that, **alert on absence** --- a record count that drops, a NULL rate that climbs, a partition that simply stopped arriving. Absence alarms are unglamorous and they are the only thing that ever catches "the job silently stopped running," which no error-rate monitor can, because a job that isn't running produces no errors. And the third, cheap and underrated: a **synthetic canary**, a known record pushed through the *real* pipeline on a schedule and asserted at the far end, which converts a silent class into a loud one and tests the whole path rather than each component's opinion of itself. The runbook is what I follow after one of those fires. Without them it is a document whose first reader is an escalation.
Int2: "Hypothesis-first, confirm before changing anything." It's 3am, the fix is one line, and the site is down. Do you really hold that line?
Yes for the **diagnosis** and no for the **mitigation**, and collapsing those two is exactly how this rule gets a reputation for being academic. There are two separate jobs in an incident --- stop the bleeding, and understand the cause --- and hypothesis-first governs the second one only. If a **rollback or a feature-flag flip** restores service, I do it immediately and I owe nobody a mechanism first, because reverting to a known-good state isn't a change based on a guess, it's the removal of a recent change. What the rule actually forbids is the thing that genuinely burns the night: **speculative forward fixes** --- bumping a timeout, growing a pool, restarting a service on a hunch. Every one of those mutates the system I am trying to read, and the outcome is bad either way: if the symptom improves I have learned nothing about why and cannot tell whether it will return, and if it doesn't I have added a variable to an already confusing picture. So the working version is: **revert freely, mitigate freely, and require a confirmed mechanism before any forward change.** And the tell that a team has quietly lost the discipline is a postmortem timeline with five changes in twenty minutes and no statement anywhere of which one actually worked.
...
Int2: The storm has stopped. Someone proposes replaying all 30,000 failed requests so no customer data is lost. What do you say?
That I do not yet know they *failed*, and until I do, that replay is the second incident. A large share of a storm like this is **timeouts**, and a timeout is an **unknown**, not a failure --- the request may have arrived, been processed and committed, with only the response lost on the way back. So "replay everything that errored" re-applies every one of them that actually succeeded, and because the amplifier was a tight loop on the same input, those duplicates arrive in bulk rather than one at a time. The shape of the damage also inverts, which is the part I would say out loud: the original incident was **loud** --- thirty thousand log lines, an alert, a channel full of people --- and the replay's damage is **silent**, showing up as doubled charges and drifted counters that a customer finds in a statement days later. That is this topic's own bug one level down: I would be trading a visible outage for exactly the kind of invisible wrongness the green dashboard taught us to distrust. So I split the population before touching it, and the split is **semantic, not numeric**, because "it starts with a 4" is the reasoning that gets this wrong: errors whose outcome is *known* (a validation rejection, a 400 or a 422 --- refused before it ran, so these genuinely did not happen and replay safely) from errors whose outcome is *unknown* (timeouts, 5xxs on a write path, and the 4xxs that only look safe --- a 408, or the client-disconnect 499s a tight retry loop manufactures by the thousand, where my handler may well have run to completion after the caller hung up) --- and for the unknown set I reconcile against the downstream's own state instead of replaying blind. If the operation had *already* carried an idempotency key while the storm was running I could replay the whole set without splitting it at all, but the key I asked for above went in **after** these 30,000, so it saves the next storm and not this pile --- and pretending otherwise is how a bad replay gets approved.
...
Model: **I go to the logs, and specifically to the gap between log error volume and the APM error rate** --- a large gap is the signature of a `try/catch` that is swallowing exceptions. The mechanism is worth saying out loud, because it explains why the dashboard is not lying so much as answering a different question: the error is **caught**, so it never reaches the top level and never produces an HTTP error status, and an error status is what APM counts. The handler logs it and returns a degraded or empty result, so APM sees a clean zero while the logs fill up and users are hurt. I'd confirm it by finding the catch blocks on the failing path and checking whether they log-and-continue rather than rethrowing or recording a metric. The fix is two-part. Make caught errors **observable** --- structured-log them with an error code *and* emit a metric, so the rate reaches alerting rather than dying in a log file nobody tails. And **stop returning success on a caught failure**, because a swallowed error that is also hidden is strictly worse than one that surfaces: it converts a loud failure into a silent wrong answer. The point I'd want to land is that **an error rate is only as honest as the error handling underneath it** --- every place that catches and does not re-surface is a blind spot in *every* metric built on thrown errors, and those blind spots are invisible by construction.
Int: Your whole detector is the gap between log volume and the APM rate. What about the catch block that logs nothing at all?
Then that detector is blind too, and I need one that does not depend on the failing code volunteering anything. Two work. **Instrument at a layer the buggy code cannot influence**: the gateway's own status and latency distribution, and the client's telemetry --- because a silently swallowed exception still produces a **degraded response**, and that response is visible from outside even when nothing inside was recorded. An empty array where a populated one belongs looks like success to APM and like a bug to the app rendering it. And **measure success rather than failure**: a swallowed exception nearly always returns empty or null where a value was expected, so a business-level metric --- recommendations returned per request, rows in the report, completion rate on the step --- falls while the error rate stays flat. That contradiction has exactly one resolution, and it is the signal I trust most, because it is derived from what users actually got instead of from what my code chose to admit. Statically, I'd also lint for it: a `catch` must rethrow, record a metric, or carry a written justification, and an empty catch block fails the build. But the framing is the durable part --- **any alarm derived from thrown errors inherits the honesty of the error handling**, so the alarms worth trusting are the ones built on outcomes.
