...
That I do not yet know they *failed*, and until I do, that replay is the second incident. A large share of a storm like this is **timeouts**, and a timeout is an **unknown**, not a failure --- the request may have arrived, been processed and committed, with only the response lost on the way back. So "replay everything that errored" re-applies every one of them that actually succeeded, and because the amplifier was a tight loop on the same input, those duplicates arrive in bulk rather than one at a time. The shape of the damage also inverts, which is the part I would say out loud: the original incident was **loud** --- thirty thousand log lines, an alert, a channel full of people --- and the replay's damage is **silent**, showing up as doubled charges and drifted counters that a customer finds in a statement days later. That is this topic's own bug one level down: I would be trading a visible outage for exactly the kind of invisible wrongness the green dashboard taught us to distrust. So I split the population before touching it --- errors whose outcome is *known* (a 4xx, a validation rejection: these genuinely did not happen and replay safely) from errors whose outcome is *unknown* (timeouts, and 5xxs on a write path) --- and for the unknown set I reconcile against the downstream's own state instead of replaying blind. If the operation carried an idempotency key I can replay the whole set without splitting it at all, which is the same key I asked for to tame the amplifier, now earning its keep a second time on the recovery path.
