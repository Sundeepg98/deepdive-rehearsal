...
The number people reach for is a bare socket buffer --- "fifty thousand times four kilobytes is two hundred megabytes, that is nothing" --- and on any connection carrying TLS that undercounts by roughly **eightfold**, before the kernel's own socket memory pushes it further. It is worth being precise about what C10K actually solved: the **event loop**. `epoll` makes fifty thousand *idle* descriptors nearly free in CPU terms, and a file descriptor itself is a table entry. What did not go away is **per-connection buffers**, and TLS dominates them --- a TLS record can be sixteen kilobytes, so each connection carries a read and a write buffer of roughly that size, about **32 KB per connection** before a byte of application state. That is precisely why nginx exposes `ssl_buffer_size` as a knob at all. Fifty thousand of those is about **1.6 GB of buffer alone**, plus kernel socket memory, plus whatever per-device session state you keep. So the honest sentence is that C10K solved the *scheduling* problem and left the *memory* problem untouched, and the right back-of-envelope is **tens of kilobytes per held connection, not single-digit kilobytes** --- which is what turns "fifty thousand sockets is free" into "fifty thousand sockets is a couple of gigabytes and a capacity plan."
