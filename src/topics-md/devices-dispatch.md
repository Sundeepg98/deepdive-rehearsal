...
Senior: Recognizing that long-poll just **relocates the connection cost from push to the device's own request** --- 50,000 idle held sockets, the C10K problem --- and that a wake-up-then-pull hybrid buys the latency by moving the sockets onto something built for them (or onto a connection the device's OS already holds) rather than by making them disappear, is the trade-off fluency a senior round wants. The arithmetic tell: C10K solved the event loop, not the buffers --- so the per-connection number is tens of kilobytes with TLS, and 50,000 sockets is gigabytes, not megabytes.
