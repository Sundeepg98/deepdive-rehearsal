...
Yes --- **a long-ish short-poll interval with jitter is the pragmatic default**, because job latency for a config rollout rarely needs to be sub-minute. If you genuinely need low latency without holding the sockets *yourself*, a **lightweight push wake-up** that just says "poll now" lets devices idle-poll slowly and only poll eagerly when told to. A **silent push notification** is the clean version: FCM/APNs multiplex over a connection the device's OS already holds, so you add no sockets at all. An **MQTT broker** is the other option if you already run one --- but be precise about what it buys, because this is where the claim usually gets overstated: a subscription *is* a held session per device, and the keepalive is only the client's PINGREQ liveness timer, not a payload, so the wake-up is a PUBLISH on a subscribed topic. You have **moved** the 50,000 connections onto a broker built for them, not eliminated them. So the honest framing is that a wake-up channel gets long-poll's latency while moving the connection cost off your dispatch tier, and the actual work still flows through the stateless pull. You reserve true long-poll for smaller fleets where the connection count is manageable.

Follow: You said "real memory pressure." Put a number on it --- what do 50,000 held connections actually cost?
The number people reach for is a bare socket buffer --- "fifty thousand times four kilobytes is two hundred megabytes, that is nothing" --- and on any connection carrying TLS that undercounts by roughly **eightfold**, before the kernel's own socket memory pushes it further. It is worth being precise about what C10K actually solved: the **event loop**. `epoll` makes fifty thousand *idle* descriptors nearly free in CPU terms, and a file descriptor itself is a table entry. What did not go away is **per-connection buffers**, and TLS dominates them --- a TLS record can be sixteen kilobytes, so each connection carries a read and a write buffer of roughly that size, about **32 KB per connection** before a byte of application state. Fifty thousand of those is about **1.6 GB of buffer alone**, plus kernel socket memory, plus whatever per-device session state you keep. That is the *untuned* number though, and the second half of the answer is knowing it is tunable: OpenSSL's `SSL_MODE_RELEASE_BUFFERS` hands both buffers back while a socket sits idle --- nginx turns it on for you --- which is how a gateway built for held connections gets back toward ten kilobytes a socket. So the honest sentence is that C10K solved the *scheduling* problem and left the *buffers* untouched, and the right back-of-envelope is **tens of kilobytes per held connection until someone has deliberately tuned it down, not single-digit kilobytes for free** --- which is what turns "fifty thousand sockets is free" into "fifty thousand sockets is a couple of gigabytes and a capacity plan."

Senior: Recognizing that long-poll just **relocates the connection cost from push to the device's own request** --- 50,000 idle held sockets, the C10K problem --- and that a wake-up-then-pull hybrid buys the latency by moving the sockets onto something built for them (or onto a connection the device's OS already holds) rather than by making them disappear, is the trade-off fluency a senior round wants. The arithmetic tell: C10K solved the event loop, not the buffers --- so the per-connection number is tens of kilobytes with TLS until you have deliberately tuned it down, and 50,000 sockets is gigabytes, not megabytes.
...
function (vals, fmt) {
  ...
  return [
    { k: 'Steady poll QPS', v: pollSec > 0 ? fmt.n(Math.round(fleetSize / pollSec)) : 'n/a', u: 'req/s', n: 'fleet size over the interval \u2014 the constant load on the stateless server just from devices asking for work', over: Math.round(fleetSize / pollSec) > 2000 },
    ...
    { k: 'Payload bandwidth (peak)', v: pollSec > 0 ? fmt.n(Math.round(fleetSize / pollSec * jobKB / 1000)) : 'n/a', u: 'MB/s', n: 'the worst case, while a rollout is draining and every poll returns a job \u2014 in steady state most polls are empty and this is near zero. Keep the document small; a fat payload times the fleet is real bandwidth, which is why a large image goes by reference', over: Math.round(fleetSize / pollSec * jobKB / 1000) > 50 },
    ...
    { k: 'Time to reach the fleet', v: fmt.n(pollSec), u: 's (one poll interval)', n: 'the enqueue is ONE bulk INSERT of ' + fmt.n(fleetSize) + ' rows \u2014 seconds, not a per-device send loop, so it is never the long pole. What paces a pull rollout is the poll itself: jittered devices collect the job across one interval, then apply. You shorten a rollout by shortening the poll, not by speeding up the insert', over: false }
  ];
}
```
