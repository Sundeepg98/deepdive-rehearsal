...
In most shops the honest answers are "thousands" and "nobody" --- which turns *quarantined* back into *lost*, and I would have to withdraw the word I just used. A dead-letter topic is only the opposite of losing a record if something **drains** it; unattended it is a durable, well-formatted, fully-preserved graveyard, and the preservation is exactly what makes it feel safe enough to ignore. That is this topic's own bug one level down: I fixed silent head-of-line blocking by relocating the silence somewhere with better retention. So the dead-letter path is not finished when it exists, it is finished when it has an **owner**, an alarm on **age-of-oldest** as well as depth, and a replay path somebody has actually run. Age-of-oldest is the load-bearing one, because a steady trickle never trips a depth threshold while still meaning nothing has been examined since spring. Two things make the replay real rather than theoretical: the dead-lettered record has to carry enough context to be re-processable --- the exception, the consumer version, the correlation id, the original partition and offset --- and the replay itself has to be **idempotent**, because every healthy record that overtook it has already been processed downstream. A dead-letter nobody watches is the ordering problem I refused to accept, quietly re-accepted with a longer retention period.
