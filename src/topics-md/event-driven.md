...
Int: You dedup on the event id in the payload. Who mints that id --- and is it the same id when the *producer* retries?
That is the hole that defeats this design most often, and it sits entirely upstream of anything my consumer does. If the producer generates the id **inside its retry loop**, every attempt carries a fresh id, my consumer sees N genuinely distinct events, and my dedup is perfect and completely useless --- which is the classic double-charge signature, and it looks like a consumer bug for as long as you're only reading consumer code. So the id has to be minted **once at the point of intent**, before any retry, and resent unchanged; better still, **derived from the business event** --- a hash of the entity and the transition --- so it is reproducible rather than remembered, and survives the producer crashing and restarting mid-flight. The same mistake has a transport-level twin, which is why the Model insists on the payload's id: **SQS `MessageId` is per-delivery**, a redelivery carries a new one, so deduping on it means never deduping anything while appearing to. And the durable fix is structural rather than a convention people are asked to remember: put the id in the **event schema as a required field**, mint it in the **shared publishing library** rather than in each producer, and where the producer writes to its own database first, use a **transactional outbox** so the row and its event id commit together and the identity is fixed before anything can retry. Consumer-side dedup can only ever be as good as the identity the producer handed it.
...
Model: **I'd size the fleet from Little's Law and then argue about what the queue is actually for, because those are two different claims.** The sizing: required concurrency is arrival rate times processing time, so at 5,000 events a second and 40ms of work each, that's `5000 x 0.04 = 200` concurrent consumers just to keep pace --- and I'd say that number out loud early, because it is the one that tells you whether the design is plausible at all. Then the queue's job: a **burst is not extra capacity, it is deferred capacity**. When the arrival rate spikes above what the fleet drains, depth grows; when it falls back, the same fleet works the backlog off, and the queue has converted a latency spike into a bounded delay instead of a dropped request. That only holds if the fleet's steady throughput genuinely exceeds the average arrival rate --- the queue absorbs **variance**, never a deficit. And I'd name the standing cost rather than let it hide: **one dedup lookup on every message**, before the work, which is the price of effectively-once and puts a store on the critical path of 100% of traffic.
Int: Little's Law gives you the average. If I staff exactly that number, what does the queue look like at five thousand a second?
Unbounded --- and that's the trap in using Little's Law as a **sizing rule** rather than as the relationship it is. `L = lambda x W` tells me the concurrency required to keep up **on average**, so a fleet of exactly 200 is running at **100% utilization**, and standard queueing results say waiting time goes to infinity as utilization approaches one. The intuition: arrivals are not smooth and service times are not constant, so every interval where the instantaneous rate runs above the mean builds a backlog --- and a fleet with **zero surplus has nothing to work it off with**, so the backlog never comes back down. A queue only drains when throughput *exceeds* arrivals, and at exactly-average it never does. So I size to a **target utilization**, 60-70%, which makes it roughly 300 consumers rather than 200 --- a real multiplier, not a rounding error. The second correction matters just as much: **`W` is not a single number.** Processing time has a tail, and a handful of slow messages occupy workers for far longer than the mean, so I'd size against something like the p90 service time, or discover that my effective concurrency is well under what the average predicted. The version I'd actually say: **Little's Law gives me the floor; the utilization target and the service-time tail turn that floor into a fleet.**
...
Model: **Per-entity ordering, not global ordering --- that distinction is the whole answer.** I partition by device id, so every event for one device lands on one partition and is consumed in the order it was written, while different devices proceed independently. Global ordering would give me the same guarantee and cost me all my parallelism, because a single total order means a single consumer; per-key ordering costs nothing, because the ordering I actually need was never between devices. So **the partition key is the design decision here**, and I'd say plainly that it is the only thing standing between me and a correctness bug --- get it wrong and no amount of consumer logic recovers the order. Two things I would not skip. Ordering is a **per-partition** guarantee, not a per-topic one, so it survives only while one device maps to one partition and one consumer holds it. And consumers still **version-check** --- carry a sequence number or a state version on the event and reject anything older than what's applied --- because ordering does genuinely break at the edges: a producer retry without idempotence, a consumer rebalance mid-batch, a redrive from a dead-letter queue. The version check is what turns those from silent corruption into a discarded duplicate. The cost I'd name up front: one device's throughput is now capped at one partition and one consumer, so a single very hot device cannot be scaled horizontally at all.
Int: You partition by device id. Six months in you double the partition count to add throughput. What happens to ordering?
It breaks --- silently, at exactly the moment I was trying to help, which is what makes this one nasty. Placement is `hash(key) % partitionCount`, so **changing the count rehashes essentially every key**: a device that mapped to partition 3 now maps to partition 11 while its earlier events are still sitting unconsumed in 3. Two consumers now hold events for the same device with no ordering relationship between them, and my version check --- which existed for the rare reorder --- either starts firing constantly or quietly drops real updates. Nothing warns me; adding partitions is a one-line administrative operation with no dialog box. So the rule I'd hold is that **partition count is part of the schema, not a capacity knob**: over-provision it at design time, since I can always add consumers up to the partition count but I cannot change the count without this. If I genuinely must expand, the safe path is to **drain first** --- stop producing, let every partition be consumed to its end, then repartition --- or publish to a new topic and cut over, which is the same idea with a rollback available. And if repartitioning has to be routine, then modulo is simply the wrong placement function and I want **consistent hashing** or an explicit key-to-partition map, so adding capacity relocates a small known set of keys rather than nearly all of them.
...
Int2: Six months on, how many records are in that dead-letter topic, and who has looked?
In most shops the honest answers are "thousands" and "nobody" --- which turns *quarantined* back into *lost*, and I would have to withdraw the word I just used. A dead-letter topic is only the opposite of losing a record if something **drains** it --- and at six months the arithmetic is worse than neglect, because a shop that never looked at it never set its **retention** either: on Kafka's seven-day default the broker deleted most of what was quarantined months ago, so what is left is a graveyard that only *looks* preserved. That appearance is exactly what makes it feel safe enough to ignore. That is this topic's own bug one level down: I fixed silent head-of-line blocking by relocating the silence somewhere that sounds like custody and, left unconfigured, runs on the very same clock. So the dead-letter path is not finished when it exists, it is finished when it has an **owner**, an alarm on **age-of-oldest** as well as depth, and a replay path somebody has actually run. Age-of-oldest is the load-bearing one, because a steady trickle never trips a depth threshold while still meaning nothing has been examined since spring. Two things make the replay real rather than theoretical: the dead-lettered record has to carry enough context to be re-processable --- the exception, the consumer version, the correlation id, the original partition and offset --- and the replay has to be safe against a world that moved on --- **idempotent**, because the poison record most likely half-executed before it died, and **version-checked**, because every healthy record that overtook it has already been applied, so a blind re-apply is a stale write rather than a duplicate. A dead-letter nobody watches is the ordering problem I refused to accept, quietly re-accepted under a better name.
