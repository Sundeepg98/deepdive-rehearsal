...
Model: **I'd elect a leader through a consensus-backed store --- etcd or ZooKeeper --- on a renewable lease, and then spend the rest of my time on the part that actually goes wrong, which is not the election.** The mechanics: only the leader processes, the others stand by; on the leader's death its lease expires, a standby is elected, and it reconciles whatever was in flight. The lease TTL is a genuine trade I'd tune deliberately --- long enough to ride out ordinary GC pauses and network blips so the system doesn't flap, short enough that failover isn't a visible outage. But the two things I'd emphasise are the ones people skip. First, **the leader must stop processing the instant it cannot renew** --- an un-renewed lease means it is no longer the leader, and continuing to work "until someone tells me otherwise" is the bug, because nobody is going to tell it. Second, and this is the load-bearing one: **election alone never gives you single-writer safety.** A leader can be paused by GC for longer than its lease, wake up still believing it leads, and act. So every action on a shared resource carries a **fencing token** --- a monotonically increasing number from the election --- and the resource rejects anything older than the highest it has seen. Election decides who *should* write; fencing is what makes it true.
Int: A fencing token only works if the resource *checks* it. My storage is S3, and it has never heard of your token. What now?
Then I do not have fencing, and the honest first move is to stop calling it that --- this is the limitation that gets glossed over, because "use fencing tokens" is repeated far more often than "the resource must participate." A plain object store accepts a write from whoever holds valid credentials; it has no notion of my lease, my epoch, or which of two writers is stale. So I'd reach for the closest thing the resource *does* offer. **Conditional writes**: S3 now supports `If-Match` on an ETag and if-none-match preconditions, so I can make the write conditional on the state I read, and a stale leader's compare-and-swap fails --- that is fencing, expressed in the vocabulary the service actually has, and it's the first thing to look for on any resource. **Make the token part of the namespace**: write to a path or object key that includes the epoch, so a stale leader physically cannot overwrite the current leader's output, and a reader resolves the highest epoch --- the write still happens, but it lands somewhere harmless instead of corrupting live state. **Route the effect through something that can check**: if the authoritative state lives in a database that supports a conditional update on an epoch column, put the token there and let the object store hold only derived data. And if none of that is available, then I'd say plainly that **I cannot prevent the double write and can only narrow and detect it** --- a lease comfortably longer than any plausible pause, a check of lease validity immediately before the call, and reconciliation behind it. The senior version of this answer is refusing to claim a guarantee the infrastructure does not provide.
...
Model: **The whole problem is that "the primary is dead" is not something any single observer can know --- it can only ever tell you it stopped hearing from it --- so the design has to be built around that uncertainty rather than around detection.** Concretely: **quorum-based failover**, where a majority of an odd-sized control group must agree before anything is promoted, because a majority cannot exist on both sides of a partition and that is precisely what stops two primaries being elected. Then **fence the old primary before or as the new one comes up** --- STONITH, revoking its write capability, or fencing tokens on the shared storage --- since a partitioned primary is still alive and still willing to accept writes, and it has no way of knowing it has been replaced. Then **choose which replica to promote**, and I'd name that as a real decision rather than a mechanical step: a caught-up replica loses nothing, and promoting a lagging async replica means accepting **bounded data loss** in exchange for faster recovery, which is an availability-versus-durability call somebody senior should have made in advance rather than at 3am. Finally, redirect clients. The invariant underneath all of it, and the sentence I'd close on: **never allow two nodes to accept writes concurrently** --- everything above exists to make that impossible rather than unlikely.
Int: Quorum stops you promoting two primaries. What stops the *clients* from still writing to the old one?
Nothing that the election did, and that is exactly the gap people miss --- **quorum is agreement among the control plane, and the data path is a separate system that was never consulted.** The old primary is up, its port is open, and a client holding a connection or a cached DNS answer keeps sending writes to it perfectly happily; it accepts them, because from its point of view nothing has happened. So the promotion is only safe if something **actively severs that path**, and the mechanisms are unglamorous and specific. **Fence at the resource**: revoke the old primary's credentials or set it read-only, which is the reliable one because it does not depend on the old primary cooperating or on clients noticing. **Fence at the network**: a security-group or firewall change, or STONITH, which is the same idea with a bigger hammer. **Move the address rather than telling clients about a new one**: a virtual IP or a proxy layer that clients connect through, so failover is a routing change and no client has to learn anything --- which is why serious setups put a proxy in front rather than relying on client reconfiguration. What I would *not* rely on is **DNS**, because a TTL is a suggestion, resolvers and JVMs cache past it, and long-lived connections do not re-resolve at all. And the general principle worth stating: **a failover is not complete when the new primary is up, it is complete when the old one can no longer be written to** --- until then you have two writable primaries and a promise.
