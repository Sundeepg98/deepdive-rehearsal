...
Follow: Put a number on it. A hundred backends, a million requests --- what does the second choice actually buy?
This is exactly where people quote the result and say a number that is wrong by a factor of about five thousand, so it is worth doing slowly. `log log n` is **not the load** --- it is the **excess over the average**. Saying "with two choices the busiest backend ends up holding about two requests" is the misstatement, and it is an easy one to make because `log log n / log 2` at a hundred bins really is about **2**. But a million requests across a hundred backends puts the *average* at **10,000**, so the busiest backend holds about **10,002**, not 2. The correct form in the heavily-loaded case is `m/n` **plus** that term --- and the genuinely remarkable part is that the term **does not grow with `m`**. With one choice there is no constant to quote at all: the excess scales like the square root of `m/n` times a log factor, so at these numbers it is `sqrt(2 x 10,000 x ln 100)` --- roughly **300**, putting the worst backend near **10,300**. So one choice gives you an imbalance that keeps climbing as traffic grows, and two choices pins it near a constant forever. That is why the honest headline is **"the imbalance stops growing,"** not "exponentially less load" --- and be careful quoting the percentage, because it moves the *opposite* way from most people's intuition. The one-choice excess grows like the **square root** of the load while the mean grows **linearly**, so the percentage win *shrinks* as traffic rises: about 3% at ten thousand requests per backend, about 0.3% at a million per backend. Every hundredfold increase in volume multiplies the absolute excess by ten and cuts the percentage by roughly the same factor. So quoting the percentage talks you out of the algorithm exactly as it starts earning its keep, and the claim worth making is the **absolute** one: with two choices you provision the mean plus a *constant*, at any scale, while one choice buys headroom that never stops growing.

Senior: Being able to state the **balls-into-bins result** precisely (exponential improvement from one choice to two; only a constant factor from two to three) *and* identifying that the win is as much about **destroying the herd across many LBs** as about the distribution itself. Most candidates know "P2C is good"; very few can say why two is enough and three is pointless. The arithmetic tell underneath it: `log log n` is the excess *over the mean*, not the load --- quote it as the load and you are off by the mean itself.
Speak: "Exact global least-connections needs state that's expensive to maintain and stale by the time you use it --- and it herds, because every LB reads the same counts and stampedes the same 'idlest' instance. Power-of-two-choices fixes both: sample two backends at random, send to the less loaded one. It needs no global ordering, each LB samples a different pair so the herd dissolves, and the imbalance above the mean stops growing with traffic instead of climbing the way a single random choice does."
...
Model: **One stable address in front of a pool of interchangeable instances --- and the word doing the work there is *interchangeable*.** Everything else serves it. So: a single **VIP** fronting the pool, and a **stateless backend** with session state in a token or a shared store, because the moment an instance holds something only it has, the pool stops being interchangeable and every other decision gets harder. The algorithm I'd choose from what I know about the workload rather than by default --- **least-connections or power-of-two-choices** when request costs vary, **weighted** when instances differ in size or I'm running a canary, plain round-robin only when both are genuinely uniform. **L7 if I need content-based routing or TLS termination, L4 if I don't**, since L7 buys per-request decisions and costs CPU on every handshake. Health is **two mechanisms, not one**: active `/health` probes on a schedule, plus **passive outlier detection** on real request failures --- the only thing that catches an instance answering probes cheerfully while failing actual work --- both with anti-flap thresholds. Churn is made safe at both ends: **slow-start**, so a cold instance isn't hammered precisely because it looks idle, and **connection draining**, so removal doesn't sever in-flight requests. And the LB tier itself is **redundant**, active-passive with a floating IP or anycast, because an availability layer that is its own single point of failure has inverted its purpose.
Int: Least-connections assumes a connection is a unit of work. My clients are all on HTTP/2 --- one connection, a hundred concurrent streams. What is your algorithm actually counting?
Almost nothing useful, and that's an important correction: **least-connections uses connections as a proxy for load, and multiplexing breaks the proxy.** With HTTP/2 a client holds one long-lived connection and puts a hundred concurrent streams down it, so an instance carrying a hundred in-flight requests and an instance carrying one look **identical to the balancer** --- both report a connection count of one. The algorithm degenerates into round-robin over *connections*, and under keep-alive that means traffic is pinned wherever the connections first landed and stays pinned, because nothing rebalances a connection that never closes. That is exactly how you get the classic HTTP/2 skew: two instances pegged, the rest idle, and a connection-count dashboard that looks perfectly even. Three answers depending on what I control. **Balance at L7 on requests rather than connections** --- terminate HTTP/2 at the balancer and make a per-*stream* decision, which is precisely the capability L7 was bought for and the reason this workload cannot be served at L4. If I must stay at L4, then **least-connections is simply the wrong metric** and I want an outstanding-request or latency signal reported by the instances themselves. And in either case, **bound connection lifetime** --- a max age or max requests per connection, jittered --- so long-lived connections are periodically forced to redistribute instead of pinning the layout forever. The general lesson: every balancing algorithm rests on an assumption about what a unit of work looks like, and it fails **silently** when the protocol changes that unit underneath it.
...
Model: **Sizing a pool is two questions --- how much load, and how much loss --- and only the second one is interesting.** The LB spreads traffic roughly evenly, so when an instance dies its share redistributes across the survivors, and the pool has to be sized so that redistribution **doesn't push anyone over**. That is what N+1 actually means: not "keep a spare" as a slogan, but enough headroom that losing a member leaves every survivor inside its own limit. The second half is **how long you are down a member**, and that number isn't free-floating --- active probes detect a dead instance in about `interval x threshold`, so a thirty-second probe needing three failures is a ninety-second window in which the survivors carry the extra load, and I get to trade that against how twitchy I'm willing to be. The replacement then arrives **cold**, so **slow-start** eases it in rather than letting least-connections hammer it precisely because it looks idle. The part I'd make explicit is that running near capacity **does not degrade gracefully**: survivors that get overloaded begin failing their own health checks, get ejected, and their load redistributes onto a pool that is now smaller still. That's the cascade, and it is why headroom is a correctness property rather than a comfort.
Int: N+1 covers losing one instance. What killed that instance --- something about the instance, or something all six of them share?
That is the assumption underneath the whole arithmetic and it deserves the challenge, because **N+1 is sized for an independent failure and most real failures are correlated.** A bad disk or a wedged process takes one instance; a **bad deploy** takes every instance running the new version, an **availability-zone loss** takes every instance in that zone, a **poison request** takes whichever instances receive it, and a **shared dependency** slowing down makes all six look unhealthy at once. In every one of those, "we can lose one" is answering a question nobody asked. So the honest version sizes against a **failure domain rather than a count**: spread across three AZs, the unit is a third of the fleet, and I want the surviving two-thirds to carry full load --- which is a substantially more expensive statement than N+1, and it is the real number. Then I'd spend effort **reducing the correlation** rather than only paying for it: distribute across zones deliberately, roll deploys **progressively with automatic rollback on error rate** so a bad version is caught at 5% of the fleet instead of 100%, and keep health checks **shallow enough that a shared dependency cannot fail them collectively** --- which is exactly the trap where a brief database hiccup ejects the entire pool and converts a recoverable blip into a total outage. The question I'd hold myself to is: for whatever headroom I bought, **which specific failure is it headroom for?** If the answer is "a random single instance," I have priced the least likely thing that will ever happen to me.
