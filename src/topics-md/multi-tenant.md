...
Model: **They stay shared because density is the whole economic argument, and at five thousand tenants the alternative is absurd on arithmetic alone.** Rows carry a `tenant_id` and every query is scoped by it, so one schema, one connection pool and one migration serve everybody --- and the marginal cost of tenant 5,001 is some rows, not a database. Going per-tenant for everyone means five thousand of everything: five thousand connection pools competing for the same backend, five thousand migrations to run and to monitor on every release, and five thousand sets of idle headroom, because each database must be sized for its tenant's *peak* while most tenants are tiny and asleep most of the time. That last one is the real bill --- you pay for the peak of every tenant simultaneously and use almost none of it. So shared is the default, and what *forces* a split is never "we got bigger," it is a **requirement the shared model structurally cannot express**: **data residency**, where a tenant's rows must physically live in a jurisdiction; a **contractual SLA or a dedicated maintenance window**, which needs a blast radius you can point at; or a per-tenant operation like **restore this tenant to last Tuesday**, which is painful to do surgically inside a shared table. Those peel the specific tenants that have them, which is usually the largest few, and everyone else stays dense.
Int: Your five thousand tenants are not the same size --- the largest is bigger than the bottom four thousand combined. What does that distribution do to "cheap and dense"?
It undermines the assumption quietly, and the reason it matters is that **SaaS tenancy is essentially always a power law** while shared-schema designs are reasoned about as if tenants were comparable. Several things break at once. The **query planner** sees one `tenant_id` predicate whose selectivity spans four orders of magnitude, and with a cached or generic plan you get a single strategy applied to both --- an index plan chosen for a small tenant that becomes a catastrophic loop for the whale, or a scan-shaped plan for the whale that is nonsense for the tail. That is the classic multi-tenant plan-instability incident, and it presents as "the same query is sometimes a thousand times slower" with no code change behind it. **Shared resources are consumed by whoever is largest**, so the whale's expensive report evicts everyone else's working set from the buffer pool and the median tenant's latency is set by a tenant they have never heard of. And **per-tenant operations get quietly expensive**: extracting or restoring one tenant means picking their rows out of a very large shared table, which is precisely the operation the big tenant is most likely to ask for. The thing that makes all of this manageable is unglamorous and usually missing: **observability broken out by tenant.** An aggregate p99 across five thousand tenants is dominated by the biggest ones and tells you nothing about what the median tenant experiences, so without per-tenant metrics you cannot even *see* this failure, let alone argue about it. Then the responses follow naturally --- per-tenant resource limits so no single tenant can consume the shared budget, attention to plan stability, and yes, peeling the whales; but peeled because **their shape broke the shared assumption**, which is a much more defensible reason than because they are important.
...
Model: **I'd resist making the tier boundary an architecture boundary, because "free" and "enterprise" are commercial categories and they should buy different guarantees, not different codebases.** Everybody starts in the shared, tenant-scoped database, with isolation enforced **structurally** --- the tenant predicate applied in one layer, from a verified identity claim rather than anything the caller can set --- so the security property does not depend on which tier you are in. What the enterprise tier actually buys is the things the shared model cannot express: **data residency**, a **dedicated SLA and maintenance window**, per-tenant backup and restore, and, where it is genuinely contracted, its **own database**. Those tenants get moved deliberately, one at a time, for a named reason. And underneath both tiers sit **per-tenant resource limits** --- query budgets, rate limits, concurrency caps --- so a single tenant's runaway usage cannot consume the capacity everyone else is relying on. The thing I would hold the line on is that the two tiers run the **same code path against the same enforcement layer**, because the moment "enterprise" means a second implementation, the shared path is the one that is under-tested and the enterprise path is the one nobody remembers to update.
Int: Your limits are per tenant, but the free tier has thousands of tenants. What stops the free tier *in aggregate* from eating the capacity enterprise is paying for --- while every single free tenant stays inside its limit?
Nothing, and that is the gap: **a per-tenant limit bounds an individual, and the thing threatening your SLA is a population.** Five thousand free tenants each behaving perfectly legally can saturate a connection pool, a database's IO, or a worker fleet, and no per-tenant rule is violated at any point --- so the dashboards show every tenant compliant while the enterprise tenant misses its latency target. Limits have to exist at **both levels**: per tenant to stop one bad actor, and per **tier or resource pool** to bound the class. The mechanism I'd actually reach for is **workload isolation rather than arithmetic** --- separate connection pools and separate worker capacity per tier, so the free tier physically cannot exhaust the resources the enterprise path uses, plus **admission control with priority**, so that under saturation the system sheds free-tier work first and deliberately rather than degrading uniformly for everyone. Uniform degradation is the failure people accidentally design: it is "fair," and it means your paying customer's SLA breaks at the same moment as your free users' experience. Then the architectural honesty. If the enterprise SLA is a **contractual number**, it is a claim about the **tail**, and limits only ever bound the *typical* case --- so on a genuinely shared resource you can make the tail *unlikely*, not *bounded*. That gap is the real argument that eventually forces a **separate compute tier** for enterprise, and it is worth noticing that this is a much cheaper move than a separate database, because it addresses the noisy-neighbour problem without splitting your data story. And one more, since the free tier is also the **abuse surface**: the limit that matters most is often on **tenant creation** itself, since per-tenant quotas mean very little if signing up for another tenant is free and instant.
...
Model: **A handler queried without the tenant predicate** --- and the root cause is not that one developer forgot, it is that the design made forgetting possible. Per-handler filtering means the security property is re-implemented at every call site and holds only while everyone remembers, forever. The fix is to move enforcement into **one structural layer** --- a repository that cannot construct an unscoped query, or row-level security in the database --- so there is nothing left to forget, plus an audit of the existing paths to find the other places it was already forgotten.
Int: You've moved enforcement into row-level security. Which connections in your system are *not* subject to it?
The ones nobody lists, and they are reliably the dangerous ones --- which is why "we turned on RLS" is an answer that sounds complete and usually is not. In Postgres specifically, row-level security does **not** apply to a **superuser**, and it does **not** apply to the **table owner** unless you have explicitly run `ALTER TABLE ... FORCE ROW LEVEL SECURITY`; roles holding `BYPASSRLS` skip it by design. Now look at which connections in a typical system run as exactly those principals: the **migration** user (usually the owner), the **analytics or BI** connection, the **background job** runner that inherited the migration role because it was convenient, and the **admin console**. Every one of them reaches tenant data, and by default several of them are outside the control you just put your faith in. The same hole exists a layer up: a repository that cannot express an unscoped query is excellent right until someone uses the ORM's **raw-SQL escape hatch**, which is exactly what people reach for when writing a report. So the deliverable is not "RLS is enabled," it is an **enumeration**: every principal that can reach tenant data, and for each one either the policy that constrains it or a statement that it has no access. Then make the structure match the enumeration --- the application connects as a role that **owns nothing** and lacks `BYPASSRLS`; jobs get their own scoped role instead of borrowing the migration role; analytics reads a tenant-scoped view or a separate export rather than the base table. And back it with two tests that run in CI: one asserting RLS is both **enabled and forced** on every tenant-scoped table, since the missing `FORCE` is the single most common version of this bug, and one **adversarial** test that authenticates as each role in turn and tries to read another tenant's rows, which is the only check that actually proves the boundary rather than describing it. The principle: **a structural control is only structural over the paths it covers**, so the work is enumerating the paths, not enabling the feature.
