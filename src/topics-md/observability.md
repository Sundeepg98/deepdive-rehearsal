...
- Instrumentation must be **safe and cheap** --- telemetry falls back to no-ops when the tracer was never initialized, and the exporter drops from a bounded queue when the collector is down or slow, so neither path can break business logic; and labels stay low-cardinality so metrics don't explode.
...
**No-op fallback factories**: if the tracer or meter isn't initialized --- a shared library used somewhere telemetry was never set up, a service that never called the SDK's setup --- the telemetry API returns objects whose methods do nothing. Be precise about what this does *not* cover: with a live SDK and a dead collector the tracer **is** initialized, so nothing becomes a no-op; spans are created and queued exactly as normal, and it is the bounded-drop exporter below that saves you. So business code calls `startSpan()` / `recordMetric()` unconditionally, with no "is telemetry available" branch, and the worst case of a telemetry outage is *missing data*, never a thrown error or a blocked request. Observability must be a strictly optional side-channel; the moment it's a hard dependency, it becomes a new way to take the system down.
...
Telemetry is a side-channel, never a hard dependency --- and it fails open on **two separate paths**, which is the distinction people collapse. If the tracer or meter isn't initialized --- a shared library used where telemetry was never set up --- the API returns **no-op objects**, so business code calls the same telemetry methods unconditionally. That is the *API* path. A **down or slow collector** is the *export* path, and no-ops do nothing for it: the tracer is initialized, so spans are created and queued as usual, and what saves you is a **bounded-drop async exporter** that discards from a full queue rather than blocking or growing. Close both and the worst case is *missing data*, never a thrown error.
...
- Safety | "Two things I insist on. Telemetry never breaks business logic -- no-op fallbacks so an uninitialized tracer does nothing, plus a bounded-drop async exporter so a down or slow collector can't block or OOM. It fails open on both the API and the export path, because a hard dependency can hide in either."
