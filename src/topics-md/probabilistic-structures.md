...
No --- it's much better than linear at first, then diminishing. For a **Bloom filter**, the false-positive rate drops *exponentially* as you add bits: each extra **~1.44 bits per element** (1/ln 2) roughly halves it --- 9.6 bits/element is ~1%, 11 bits is ~0.5%, 12.5 bits is ~0.25% --- so a few more bits buys a big accuracy gain. For **HyperLogLog**, the standard error is ~1.04/sqrt(registers), so error falls as 1/sqrt(memory) --- to halve the error you 4x the registers. So you buy a lot of accuracy cheaply at the start and hit diminishing returns; you never pay linearly, which is exactly why a tiny structure can be so accurate.
...
Yes --- **information-theoretically**, any structure with false-positive rate p needs at least ~log2(1/p) bits *per element* (= 1.44 * ln(1/p)), regardless of design: at p = 1% that floor is **6.6 bits**. A Bloom filter at optimal k uses **m/n = -ln p / (ln 2)^2 = 2.08 * ln(1/p)** --- **9.6 bits** at 1% --- and the ratio between them is exactly **1/ln 2 = 1.44x**, so Bloom sits **~44% above the floor at every p**, not just at 1%. That constant gap is precisely why **cuckoo and quotient filters**, which get closer to the bound, can be more space-efficient at low p. So ~10 bits/element for 1% isn't arbitrary --- it is 44% above a fundamental floor of 6.6, and "just use fewer bits" has a hard limit set by the rate you need.

Senior: Knowing that exceeding the planned n degrades the false-positive rate steeply (and the fixes: size for max, scalable Bloom, or rebuild), plus that bits-per-element has an **information-theoretic floor** of ~log2(1/p) --- 6.6 bits at 1%, with Bloom's 9.6 sitting exactly **1/ln 2 = 1.44x (~44%) above it** --- is the sizing rigor that separates tuning a filter from reciting the formula.
...
Because each row's counter is an **upper bound** (the element's own increments plus collision noise, and collisions only add), so every row is >= the true count. The **minimum is therefore the tightest upper bound** --- it picks the row where this element suffered the *fewest* collisions. An average or median would blend in the more-collided rows and overestimate *more*. Note what they would *not* do: since every row is an upper bound, **every** order statistic of them --- min, median, max --- is also >= the true count, so a median does not break the one-sided guarantee, it is just a looser bound. (The "a median can underestimate" fact is real, but it belongs to the **Count Sketch**, where random +/- signs make each row's estimate two-sided.) The min is what makes the estimate both as-tight-as-possible AND provably >= true --- the whole design rests on "every row is an over-estimate, so take the smallest."
...
Senior: Explaining that the **minimum** is chosen because every row is an upper bound (so the min is the *tightest* of them --- a median or mean stays >= true as well, it is just looser), and knowing the **conservative-update** variant trades mergeability for a tighter estimate, is the mechanism depth that shows you understand why the structure is built the way it is.
...
You **invert the error formula** from the tolerance the use case gives you. Bloom: pick the false-positive rate whose wasted-work cost you can eat (say 1%), then m/n = -ln(p) / (ln 2)^2 ~ 9.6 bits/element sets the memory and k = (m/n) * ln 2 ~ 7 the hashes. HLL: pick the tolerable relative error (say 2%), then registers m ~ (1.04/error)^2 ~ 2700 -> round up to 2^12, giving the KB. Count-Min: pick epsilon (overshoot as a fraction of the total count) and delta (confidence), then w ~ e/epsilon and d ~ ln(1/delta). So budget -> parameters -> memory is a direct computation, and you size for the **worst-case load** (max n, peak stream) so the error holds under the conditions that actually stress it.
...
Int: An attacker knows you're using a Count-Min Sketch. Can they use the collisions against you?
Yes, and this is the failure that separates a textbook answer from a deployable one, because everything above assumes collisions are **random** --- and against an adversary they are **chosen**. The sketch's hash functions are typically fixed and, in most libraries, publicly known. If an attacker can determine which cells a target's key maps to, they can deliberately send traffic under keys that **collide with that target in every row**, driving its estimate above the threshold while the target does nothing wrong. That inverts the whole safety argument: I justified the structure on "it can only over-count, and over-counting an abuser is fine," but a *chosen* over-count is a **denial of service against a specific victim**, delivered through my own defence. The same trick works in reverse for evasion --- spread abuse across many keys so no single one crosses the line, since the sketch only sees per-key frequency. The fix is the standard one for adversarial hashing and it needs to be there from the start: **keyed, randomized hash functions with a secret seed** chosen at startup and unknown to clients, so the attacker cannot compute which cells anybody occupies; **rotate the seed** periodically, which also has the useful side effect of aging the sketch. Beyond that, defence in depth: **verify at the boundary** with an exact counter before taking an irreversible action against a client --- which I wanted anyway for false positives and which now doubles as the check that defeats a manufactured collision --- and **alarm on the shape of the estimates**, because a sudden cluster of clients arriving at the threshold together is a much more likely signature of an attack on the sketch than of genuine simultaneous abuse. The generalisable lesson: **a probabilistic structure's error guarantee is an average over random inputs, and an attacker is not a random input** --- so any sketch on a security path needs a secret in its hash, or its guarantees are advisory.
