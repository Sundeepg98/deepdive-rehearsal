...
That's the catch. Splitting `key` into `key:0..key:N-1` and hashing each request to a sub-key spreads the write load, but now no single counter knows the client's total. You give each sub-key a share (limit/N), and the error that introduces runs in a *specific direction* worth naming out loud: the N slices **sum to exactly the limit**, so the client can never *exceed* it --- sub-key sharding is structurally conservative. What it does instead is **under-admit under skew**: a client whose requests hash unevenly exhausts one sub-key and starts getting rejected at limit/N while the cold sub-keys sit idle, so you throttle someone who was under their real cap. You fix that by periodically **aggregating** the sub-counters into a true total, or by adding **headroom** to each slice --- and note it is the headroom, not the sharding, that buys you an over-admission. The honest framing: sharding trades exact global accuracy for throughput, and I'd say *which way* it is wrong rather than wave at "approximate."
...
Senior: Knowing that *both* mitigations (sub-key sharding, local-bucket sync) convert an exact global limit into a bounded-approximate one --- and quantifying the error *with its direction*, rather than presenting sharding as a free fix. Sub-key sharding errs **strict**: the N slices sum to exactly the limit, so it can never overshoot, but a fully-skewed client collapses to limit/N and gets throttled under its own cap. A local bucket with sync errs **loose**: an overshoot of (local allowance x instances) per sync interval. Naming which way each one is wrong is the Staff-level honesty.
Speak: "A hot key funnels every request to one shard, so the counter is a throughput bottleneck. You shard into sub-keys or give each instance a local bucket that syncs periodically --- but both turn an exact global limit into a bounded-approximate one, and they're wrong in *opposite* directions: sharding under-admits a skewed client, because the N slices sum to the limit, while the local bucket over-admits by local-allowance times instances per sync interval."
...
A truly global limit needs a shared cross-region counter, which adds inter-region latency to every request. The common alternatives are to let each region enforce the limit against its **own local counter** (accepting that a client spread across all R regions reaches R times the global cap) or to route a client consistently to one region so its counter stays local. The trade is global accuracy against cross-region latency.

Follow: Each region enforces the full limit against its own local counter, so a client that hits all R regions gets R times the intended global cap. Is that acceptable?
It depends on what the limit is *for*, and you say so explicitly. For most protective rate limits, independent per-region enforcement with the accepted R-times worst case is fine --- a client would have to deliberately spread traffic across all regions to reach it, and even then each region's local limit still protects that region's capacity, which is the thing that actually melts. You're trading a bounded, worst-case global over-admission for zero cross-region latency on the hot path. And I'd pre-empt the obvious "so just divide it by R": a **static limit/R split cannot overshoot** --- the R slices sum to exactly the limit --- but it is *worse*, because **geo-routing means most clients live in one region**, so a perfectly normal client gets capped at limit/R, R times too strict, in order to defend against an attack nobody is running. That is why you don't statically divide. You either accept the R-times worst case, **pin** the client to a home region, or reconcile a *dynamic* allowance. It's *not* fine when the limit is a hard contractual or billing boundary ("exactly 1000/day, period") --- there R-times is a real violation, and you need a true global counter or client-to-region pinning. Match the mechanism to whether the limit is protective (independent per-region is fine) or contractual (needs global accuracy).
...
Senior: Deciding independent-per-region vs one global counter by whether the limit is *protective* (a bounded R-times slop is fine) or *contractual* (R-times is a violation) --- *and* knowing why the obvious patch is not the middle: a static limit/R split can't overshoot, but geo-routing puts most clients in one region, so it would throttle a normal client at limit/R. The real middle is pinning, or async cross-region reconciliation with local enforcement and a bounded per-interval overshoot. Rejecting the naive split for the right reason is what lands the Staff signal.
Speak: "A true global counter adds cross-region latency to every request, so you usually let each region enforce against its own local counter and accept that a client spread across R regions could hit R times the global cap --- fine for a protective limit, not a contractual one. And I wouldn't 'fix' that by statically splitting into limit/R: geo-routing puts most clients in one region, so I'd be throttling a normal client R times too hard. The middle ground is async reconciliation: enforce locally, roll counts up between regions every interval, accept a bounded overshoot."
...
You mitigate two ways, and both convert an *exact* global limit into a *bounded-approximate* one --- but they are approximate in **opposite directions**, which is the part to say out loud. **Sub-key sharding** splits the key into `key:0..key:N-1`, hashing each request to a sub-key with limit/N each: the N slices sum to exactly the limit, so it can never *overshoot*, but a client whose requests hash unevenly exhausts one sub-key and is throttled at limit/N while the cold ones idle --- so it errs **strict**, and you periodically aggregate the sub-counters for a true total. Or a **local bucket per instance** that syncs a slice of the global limit periodically: that one errs **loose**, with an overshoot bounded by (local allowance times instances) per sync interval. Either way you accept quantified slop to keep the hot path off one shard --- and you name the slop, and its direction, rather than presenting sharding as free.
...
Its counter is a **hot key** on one shard --- a throughput bottleneck. Shard into sub-keys, or a local bucket with periodic sync --- both trade exact global accuracy for throughput, and they err in *opposite* directions: sharding under-admits a skewed client (the N slices sum to the limit, so it cannot overshoot), the local bucket over-admits by (local allowance x instances) per sync interval.
...
- Per-region local counters: each region enforces the full limit itself --- fast, no cross-region hop, but a client spread across R regions can reach R times the global cap
- Static limit/R split: cannot overshoot (the R slices sum to the limit) --- but geo-routing puts most clients in *one* region, so it throttles a normal client R times too hard. Not the fix it looks like
- Global counter: one true count across regions --- accurate, but inter-region latency on every request

Enforce independently per region and accept the bounded R-times slop for *protective* limits; pay for a global counter --- or async cross-region reconciliation, which is a *dynamic* split that shrinks a region's allowance as the client burns budget elsewhere --- only when the limit is *contractual* and R-times is a real violation.
...
- THE CATCH | sub | But now no single counter knows the client's total, so limit/N is an **approximation** --- and I'd name which way it's wrong. The N slices **sum to exactly the limit**, so sharding can never let the client *overshoot*; it errs **strict**. Under skew the client exhausts one sub-key and gets rejected at limit/N while the cold sub-keys sit idle --- I've throttled someone who was under their real cap. For a true global limit I periodically aggregate the sub-counters.
- LOCAL BUCKET | sub | The alternative is a **local bucket per instance** that syncs a slice of the global limit periodically --- the hot path never touches a shared counter. That one errs the *other* way, **loose**: a bounded overshoot of (local allowance times instances) per sync interval.
- QUANTIFY | sub | Either way I'd **name the slop out loud**, with its direction: I'm converting an exact global limit into a bounded-approximate one, here's the bound, and here's whether it bites the client or the service. That honesty is the point --- I'm not pretending sharding is free.
...
- CLOSE | close | So: a hot key bottlenecks one shard; I shard into sub-keys or use local buckets with periodic sync, both of which trade exact accuracy for throughput in opposite directions --- sharding under-admits a skewed client, local buckets over-admit between syncs --- and I state the bound rather than hand-wave it.
...
- DISTRIBUTED IS APPROXIMATE | sub | **Every scaling trick makes it approximate.** Sub-key sharding, local fallback buckets, independent per-region counters --- all trade exact accuracy for throughput, each with a bounded error I can name *and a direction I know*: sharding errs **strict** (the slices sum to the limit, so a skewed client gets throttled early), while local buckets and independent per-region counters err **loose**. That's usually fine for a protective limit and *not* fine for a contractual one, so I'm honest about which I have.
...
function (vals, fmt) {
  ...
  return [
    ...
    { k: 'Peak ops at 10x burst', v: fmt.n(rps * 10), u: 'ops/s', n: 'a 10x spike over average \u2014 the TOTAL store load, which spreads across shards by key hash. It is NOT what one shard absorbs: past ~100k ops/s it simply stops fitting on one node, so this is the number that says CLUSTER THE STORE. The single-shard question is the hot key below', over: rps * 10 > 100000 },
    { k: 'Counter memory', v: fmt.n(Math.round(clients * bytesPerKey / 1e6)), u: 'MB', n: 'one ' + bytesPerKey + '-byte counter per active client \u2014 even a million clients is only ~' + fmt.n(Math.round(1e6 * bytesPerKey / 1e6)) + ' MB, so memory is never the constraint. The op rate is', over: false },
    ...
    { k: 'Added round-trip', v: '~1', u: 'ms', n: 'one atomic store round-trip on the request path \u2014 sub-millisecond in-AZ, call it ~1ms once you add the client library and a cross-AZ hop. Cheap, but it is on EVERY request, which is why the store call gets a tight timeout, a circuit breaker and a local fallback bucket', over: false }
  ];
}
```
...
Model: **Two numbers, and they point in opposite directions --- which is the whole argument.** **Memory is a rounding error.** A counter is a key and a small integer, call it a hundred bytes with overhead, so even a million distinct clients in a window is about **100 MB**, and ten million is a gigabyte. Nobody needs to design around that. **Throughput is the entire problem.** Every request must consult the limiter *before* it is served, so the store takes **at least 50,000 operations a second** as a floor --- and if the implementation does the increment and the expiry as two separate commands, that is **100,000**, for no benefit. So the design rules follow from op-rate, not capacity: make it **one atomic round trip** per request, a single native command or a small server-side script, both because two commands double the load and because a non-atomic check-then-set is a race that lets a client exceed the limit under exactly the concurrency the limit exists for. Then the consequence people skip: this store is now on the critical path of **one hundred percent** of requests, so **its latency is added to every request you serve and its availability is your availability** --- which means it wants to be an in-memory store, co-located in the same region or availability zone, and it needs a decided failure policy rather than an improvised one. The memory argument is real; it is just not the interesting half.
Int: You've argued the fleet does 50,000 a second. But the load on this store isn't uniform, is it?
No, and the non-uniformity is the part that actually breaks it. Limits are counted **per client**, so all of one client's traffic lands on **one key** --- and a key is the atomic unit, so you cannot shard your way out of it the way you can with a uniformly distributed workload. That gives you a single hot shard whose throughput ceiling is one node's, no matter how large the cluster is. And it is worse than an ordinary hot-key problem because of **who** the hot key is: the client hammering you hardest is, by definition, the one the limiter exists to stop --- so the load spikes **precisely when the limiter matters most**, and an attacker gets to *choose* your hot key. A limiter that falls over under the traffic it was deployed to stop is decoration. Three mitigations, and I would reach for them in this order. **Reject the obvious floods before the limiter sees them** --- edge or WAF admission control on IP or ASN --- so an unsophisticated volumetric attack never reaches a counter at all, which is much cheaper than counting it accurately. **Shard the hot key itself**, `client:0..k` with the budget divided k ways, accepting some drift at the boundary in exchange for spreading one client across k shards. And **enforce locally first**, so the common case needs no shared state at all. The framing worth carrying: **a rate limiter is a shared resource that an adversary is allowed to aim at**, so it has to be designed against a deliberately skewed load, not the average one.
Int2: You said "enforce locally first." My limit is 1,000 a second and I have 40 gateway nodes. What is the client's actual limit now?
Somewhere between 40,000 and 25, depending on a choice I have to make explicitly --- which is the honest way to open this, because the naive versions are both wrong. If each node enforces the **full** 1,000 locally, a client spread across all forty gets **40,000**, and the limit means nothing. If I divide and give each node **25**, then a client whose connections happen to pin to one node is throttled at 25 while thirty-nine nodes sit on unused budget --- so the limit is now a function of your load balancer, and it will be wrong for almost everybody, because traffic is never distributed as evenly as that arithmetic assumes. The workable middle is **lease-based quota distribution**: a central allocator hands each node a **slice of the budget for a short interval**, the node enforces it locally at zero network cost, and it renews or returns the unused portion when the interval expires. That gives one round trip **per interval** instead of per request, which is the entire point, while keeping the global limit approximately honest and letting hot nodes draw a bigger share than idle ones. It also degrades in a defined way: a node that cannot renew its lease falls back to its last known slice and then to a **conservative floor**, so a partition makes the limiter stricter rather than absent. Underneath all of this is the thing worth saying out loud, because it stops the whole argument being circular: **exact distributed rate limiting would require consensus on every single request**, which no one can afford at 50,000 a second. So **every real limiter is approximate**, and the actual design question is never "is it exact" but **which direction it errs, by how much, and what happens to that error during a partition** --- over-permissive is a cost and a risk you are choosing, over-restrictive is an outage you are choosing, and for a public API those are very different bets.

### DESIGN | A public API with paid tiers

Task: design the limit policy across plans and endpoints.
Model: **A sustained rate plus a burst allowance per tier, limits per endpoint class rather than one global number, and a response contract clients can actually program against.** The **token bucket** is the shape I want, because it separates the two things a single "requests per second" conflates: the **refill rate** is the sustained throughput a tier is sold, and the **bucket depth** is how much burst it may absorb. That matters because real clients are bursty and legitimate --- a nightly sync or a page load firing twelve calls at once is not abuse --- and without a burst allowance you either reject normal behaviour or you raise the sustained rate for everybody to accommodate it. Limits are then **per endpoint class**, since a search or an export costs orders of magnitude more than a status read, and one number across all of them is necessarily wrong for most; where costs vary a lot within a class, I'd rather charge **weighted cost** against the bucket than count requests. Crucially, I'd get the numbers by running the limiter in **observe mode first** --- counting and reporting without enforcing --- and set each tier above the measured legitimate P99, because a limit derived from real traffic is defensible to a customer and a guessed one becomes an incident on launch day. And the policy is part of the **API contract**, not an implementation detail: documented limits, `RateLimit-Limit` / `-Remaining` / `-Reset` headers on every response so a client can pace itself before it is rejected, and a `429` carrying `Retry-After`.
Int: What do you expect a client to actually *do* with that `429`? Because the naive ones retry immediately.
They do, and that is how a rate limiter becomes an **amplifier** for the load it was deployed to shed --- so this has to be designed for rather than hoped about. The first requirement is that **rejection must be cheap**: the `429` gets decided at the **edge**, before authentication, business logic, or any database call, because if a rejected request costs nearly as much to serve as a real one, you have saved nothing and merely changed the status code. Second, **jitter the `Retry-After` server-side**. If every throttled client is told to come back in exactly sixty seconds, you have **synchronised** them into a thundering herd that arrives together, gets throttled together, and returns together --- a self-sustaining oscillation that looks like an attack. Spreading the values breaks the convoy, and it costs one line. Third, **you cannot trust clients**, so there needs to be a consequence for ignoring the header: a client that keeps hammering through `429`s gets escalating penalties and eventually a temporary block, which is the only lever that reliably works on badly-behaved code you do not control. And fourth, a distinction that is worth getting right because it teaches clients the correct reflex: **`429` means "you exceeded your quota"** --- back off, this is about you, retrying harder will never help --- while **`503` means "we are overloaded"** --- this is about us, retry with backoff and it probably will. Returning one when you mean the other trains exactly the wrong behaviour into every integration. The underlying point is that **rate limiting is a control loop with the client inside it**, so the design is not finished at the moment you decide to reject something; it is finished when you have reasoned about what the rejection causes.
Int2: A customer on the paid tier just signs up for eight more accounts and gets eight more quotas. Is your limit real?
Not as described, no --- and that is a design error rather than a customer misbehaving, because **a limit is only as strong as the identity it counts against**, and I counted against something cheap to mint. If the quota attaches to an **API key** and keys are free, then the limit is trivially arbitraged by generating more of them; the same applies to accounts, to OAuth clients, and to anything else self-service. So the limit has to be keyed on the **entity the business model actually charges** --- the paying organisation or billing account --- with individual keys drawing from that shared pool rather than each carrying their own. Then more keys give a customer better *observability and rotation*, which is what keys are for, and no additional capacity, which is what they were never meant to grant. Aligning the limit's identity with the billing identity also settles the ambiguous cases sensibly: an enterprise customer with forty internal services genuinely is one customer, and should be able to divide its own pool however it likes. Two related edges are worth naming. **Detection still matters**, because organisations can be split too, so I would watch for correlated signups --- shared payment instruments, shared domains, identical usage fingerprints appearing at the same moment --- as an abuse signal rather than pretending the identity choice closes it completely. And **unauthenticated endpoints have no identity at all**, so login, signup and password reset can only be limited by things a client can partly forge; IP is weak on its own behind carrier NAT, so those paths want additional friction --- proof of work, a challenge, or tighter per-IP-and-per-target limits --- because that is exactly where an attacker goes once the authenticated surface is properly bounded.
...
Int2: Forget the outage. Redis is perfectly healthy and a single `INCR` times out. Did that request count?
I do not know, and saying so plainly is the answer --- a timed-out `INCR` is an **unknown**, not a failure. It may have executed on the server with only the reply lost on the way back, so the counter may already have moved. That matters because `INCR` is **not idempotent**: a client library that helpfully retries it debits the same request twice. And the direction of that error is the one that bites --- the client is now over-counted, so it collects 429s for traffic it never sent, and that reaches me as a support ticket about phantom throttling rather than as an alert, because nothing on my side ever looked broken. So the limiter's store call is retried **never**: on a timeout I fall through to the local bucket exactly as if the store were down, and I let that one increment be lost rather than risk applying it twice. Under-counting is the safe direction for a limiter --- the worst case is one extra admitted request --- and the moment under-counting stops being safe is the moment the number is **money**, at which point a bare counter is the wrong primitive and the increment has to carry a request id so a replay is a no-op. Which is the same rate-limit-versus-quota line as before, arriving from the retry side instead of the recovery side.
...
Int: Isn't "coarsely bounded by a local bucket" still instances-times the limit during the outage? How is that safe under a spike?
No --- and doing the arithmetic out loud is the answer, because that premise is the trap. The overshoot is **(local allowance x N)**, not (limit x N). Each fallback bucket is sized at **limit/N with headroom**, so a client spread across all N instances gets N x (limit/N) --- roughly its **intended cap plus the headroom factor**, not a multiple of it. You only reach N times the cap if each local bucket holds the **full** limit, and that is the naive per-instance counter, not a fallback --- which is exactly why you size the fallback as a *slice* rather than the whole number. What I have actually lost is **accuracy, not the cap**: the slices are static, so a client whose traffic piles onto three of ten instances is throttled at 0.3x its cap while the other seven sit idle --- and the headroom I add to stop that false throttling *is* the over-admission I'm accepting. So the honest bound is "the cap plus headroom, for the duration of the outage," and each local bucket still stops a single client from melting the instance it lands on, which is the thing that actually fails first. If even that is unacceptable for a given endpoint, that endpoint shouldn't fail open --- it should fail closed and eat the availability hit, which is the whole point of making fail-open-vs-closed a *per-path* decision rather than a global one. And I'd shorten the exposure anyway: a circuit breaker flips to local fast, and the store's own HA (replicas, failover) makes a thirty-second total outage rare.
