...
The mechanisms that add resilience cause the outage when naive. **Retry storms**: a dependency slows, callers retry, the retries add load, it slows more --- a feedback loop, multiplied in layered systems (three layers each making three attempts is up to 27x load on the bottom). **Cascading timeouts**: a slow dependency ties up upstream threads until the upstream can't serve anything, even requests that don't touch it.
...
Int2: Single-flight is now sitting in front of that key. What did adding it break?
Two things, and it is the second one that actually ships. First, it converts N independent requests into **one shared fate**. Before, a hundred concurrent misses were a hundred independent attempts and a flaky origin failed some of them; now the single in-flight call is the *only* attempt, so when it fails or times out, all one hundred waiters get that same failure at the same instant --- I de-correlated the load and re-correlated the outcome. It also hands every waiter the leader's timeout: a caller with a 200ms deadline is now parked behind a leader configured to wait five seconds, and cannot fail any faster than the leader does. So the waiters have to keep their **own** deadlines rather than inherit the leader's, and the waiter set needs a bound, because one wedged key otherwise accumulates waiters until it is a memory leak. Second, and worse because it is silent: **coalescing is only correct when the callers genuinely want the same answer.** The single-flight key has to include everything the response varies on --- the tenant, the authorization scope, the locale, the flag bucket --- and the moment someone keys it more coarsely than the response actually depends on, one caller's answer is handed to the other ninety-nine. That failure emits no error, no latency spike and no alert; it looks exactly like a cache hit, and it is a cross-tenant data leak. The rule I would state: a single-flight key must be at least as specific as the response it protects, and the coalescer carries its own bound and its own deadline instead of borrowing the leader's.
