...
Model: **Order the steps first, then hang the machinery off that.** I'd sort the three by reversibility: reserving inventory is cheap and silently released, charging the card is reversible but *visibly* -- a refund costs fees and the customer sees it -- and handing a parcel to a courier cannot be undone at all. So it runs reserve, charge, ship, and the **pivot** sits immediately before the shipment: everything ahead of it is compensatable, everything past it must be **retriable**, never compensatable. That one ordering decision determines which compensations I even have to write. Then each piece of machinery answers one named failure. Each step is a **local transaction** in its own service with a compensation -- release, refund, cancel -- because no transaction spans the three databases. Every step *and every compensation* is idempotent on `(saga_id, step_id)`, because the coordination is at-least-once and a re-issued charge that isn't keyed is a double charge. Step events leave through a **transactional outbox**, committed with the effect, because a `commit()` followed by a separate `publish()` is the dual write and loses the event on any crash between them. The order row sits at **`PENDING`** as a semantic lock so nothing downstream treats an in-flight order as final. And a **durable orchestrator** drives it, persisting "about to dispatch" *before* dispatching, so a crash resumes the saga rather than orphaning it.
Int: Every step is idempotent on `(saga_id, step_id)`. So where does `saga_id` itself come from -- and what stops one customer from starting two sagas for the same order?
Nothing in what I just described, which is the gap worth naming out loud: **step-level idempotency is scoped *inside* a saga and says nothing about duplicate sagas.** Two sagas for one order are each internally perfect -- different `saga_id`s, so every key differs and every conditional write legitimately succeeds -- and they cheerfully reserve twice and charge twice, leaving a saga log that looks completely healthy from both sides. So the saga needs an **identity of its own, minted outside it** and claimed atomically before instance one exists: a client-supplied request id on the submit, or better a **natural business key** -- cart id plus version, or the order id the client already chose -- so a resubmitted POST finds the existing saga and returns *its* status instead of starting a rival. The shape to recognize is that **a saga is itself an operation**, so it needs exactly the treatment its own steps get, one level up. Miss it and you have built exactly-once steps inside an at-least-once workflow, which buys you a double charge with an immaculate audit trail.
Int2: The orchestrator is now on the critical path of every order. What happens when it's down -- not crashed and recovering, but down for an hour?
Two different damages, and people cost only the first. **New orders** cannot start, so the decision is whether you reject them outright -- honest, and checkout is now down -- or durably capture the *intent* at the edge and let the orchestrator drain the queue on recovery, which keeps the storefront up and converts downtime into latency. For orders I'd take the queue. The damage people miss is the **in-flight** sagas: they are frozen **mid-transaction**, and a saga stopped between step 1 and step 2 is holding **reserved inventory**, while one stopped between 2 and 3 is holding reserved inventory **and the customer's money**. So an orchestrator outage is not a read-only outage -- it silently accumulates half-finished financial state that grows every minute it lasts, and stock that nobody else can buy. That is why durable state and a rehearsed resume path are the load-bearing parts, why semantic locks need a **lease and a sweeper** rather than a bare flag, and why I'd watch **saga age** during the incident rather than reading about it afterwards. If "resume every instance from its persisted step" has never actually been exercised, the hour of downtime is the least of the problems.

### DESIGN | Choosing choreography vs orchestration for a multi-step saga

Task: pick and justify a coordination style.
Model: I'd commit rather than hedge, and I'd use a sharper rule than counting steps: **choreography for reactions, orchestration for transactions.** Where several services are each independently *reacting* to something that already happened, events are right and the decoupling is real -- no coordinator to build or operate, no shared component whose outage stops every flow, and a team can subscribe without asking anyone's permission. Where there is **one business outcome that has to reach a terminal state**, somebody has to own driving it there, and *"the event graph"* is not an owner: the moment you need to answer "where is order 12345 and what is it waiting on," you need a coordinator with durable state you can query. The tell I'd actually apply is **compensations** -- if a step has an undo, that flow has an outcome, so it wants an owner. Counting steps fails because the two shapes look **identical on day one**: "payment reacts to order-created" is also what step one of a nine-step choreographed saga looks like, and choreography's bill only arrives later, once the flow has grown and no place knows it. Whichever I pick, three things are non-negotiable -- a **saga/correlation id on every message**, **durable state**, and an **outbox with idempotent consumers**. And if I'm orchestrating I'd **buy** the durable-execution machinery, Temporal or Step Functions, rather than hand-roll it, because the retry-and-recovery loop *is* the product.
Int: Real flows aren't purely one or the other -- placing an order is a transaction, but it also fires notifications, loyalty points and analytics. Where exactly do you draw the line inside a single flow?
At a question you can answer per step: **does this step's failure have to change the outcome?** If yes, it is a **step** -- it belongs inside the saga, with a compensation and a position in the ordering. If no, if the order is still placed whether or not it happens, it is a **reaction** and it belongs *outside*, choreographed off the single `OrderPlaced` event the saga emits on reaching its terminal state. So the saga stays small and is exactly the set of things allowed to fail the order: reserve, charge, ship. Notifications, loyalty and analytics hang off the terminal event, and none of them can unwind a shipment because none of them is in a position to. The failure mode of drawing it wrong is concrete: put loyalty points *in* the saga and a points-service outage now compensates a parcel that has already left. There's a second reason to keep the orchestrator lean -- every step inside it is one more thing whose owning team has to negotiate a change to a central component, which is exactly the coupling people fear. Autonomy over *what*, central over *when*, and the line is drawn on whether a failure is permitted to matter.
Int2: You've orchestrated it and it's live. Six months later the business wants to reorder the steps and add one -- while thousands of sagas are in flight. How does that ship?
Carefully, because **you cannot change the shape of a workflow that is already running.** An in-flight instance has a persisted position that only means something against the definition it started under, so a deploy that reorders or renumbers steps reinterprets that position and drives live orders down the wrong path -- compensating a step that never ran, or skipping one that did. So saga definitions are **versioned**, every instance is **pinned to the version it started under**, and both run side by side until the old cohort drains. That is also why bounded saga lifetimes and a stuck-saga alarm are what make this tractable: an instance that never terminates makes the old version immortal. Two specifics carry the risk. **Never renumber or reuse `step_id`s across versions** -- they are idempotency keys, so a recycled id collides with a participant's stored claim from the old definition and a genuinely new step gets silently skipped as a duplicate. Append new ids, retire old ones permanently. And if a change truly must reach running instances, a regulatory fix say, that is not a deploy but a **migration**: an explicit mapping from every reachable old state to a new one, run deliberately, with anything unmappable going to a human queue. Workflow engines ship versioning primitives precisely because this is the problem every team hits second, immediately after the happy path works.
...
Int2: The tracking consumer is live and the team opens it during every incident. What breaks?
It **lags --- and a lagging tracker does not look lagging, it looks authoritative.** The tracker is just another consumer of the same streams, so under exactly the conditions that produce incidents (a broker backing up, a rebalance, a slow downstream) it falls behind, and its answer to "where is order 123?" comes back "step 3, waiting on payment" when the order is actually at step 7 and healthy. That is *worse* than the state it replaced, because "we don't know" sends someone to go and look, while a confident wrong answer gets acted on --- somebody compensates an order that already completed, or hand-crafts an event to unstick a saga that was never stuck, and the forensic tool has become an incident source. So it has to be honest about its own freshness: every answer carries the offset and timestamp it was computed from, and it **refuses to answer** past a staleness bound rather than answering stale. The subtler failure is that it is the only component subscribed to all nine services' events, which makes it the thing most likely to break when any one of them changes an event schema --- so the system whose job is to explain an outage is disproportionately likely to be broken during one. Both point the same way: keep it a dumb projection with no business logic, version its event contracts like any other consumer, and alarm on **its own lag** as a first-class signal rather than only on the sagas it watches.
