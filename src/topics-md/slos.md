...
# (2% -> 5% -> 10%): a rule that pages after spending more budget than the
# rule that tickets has the escalation backwards.
...
Model: **Start from what the user experiences, not from what the infrastructure reports.** So: pick a user-centric SLI (proportion of successful payment requests, measured at the edge), set an achievable-but-meaningful SLO (e.g. 99.95% over a rolling 30 days -- high because payments matter, but not aspirational five-nines), derive the error budget (1 - SLO), alert on multi-window burn rates (fast burn pages, slow burn tickets), and agree an error budget policy (ship while budget remains, freeze on exhaustion); account for critical-path dependencies (the processor's SLO caps yours) by decoupling where possible.
Int: You said "proportion of successful payment requests." Define successful --- precisely, because that definition is the SLO.
It is, and this is where the real argument lives, because every number after it is downstream of this sentence. Four decisions have to be made explicitly. **Latency belongs inside the success definition.** A payment that returns `200` after thirty seconds is a *failed* payment from the user's point of view --- they gave up, refreshed, or paid elsewhere --- so the SLI is "returned a correct response **within N milliseconds**", not merely "returned a response." Availability and latency measured as separate SLOs let a service be technically green while being useless. **Client errors need a decision, not a default.** Excluding `4xx` is conventional and mostly right, but it hides a real failure mode: a `400` caused by *your* API change, a validation rule that regressed, or an expired token your own refresh flow failed to renew is your outage wearing the client's status code. So I would exclude client errors and **alarm separately on the 4xx rate**, because a spike there is your problem even though it is not in your SLI. **Measure at the edge.** Instrumenting inside the application makes it structurally incapable of seeing the failures that never arrive --- a broken load balancer, a DNS or certificate failure, an exhausted connection pool --- and the worst outage is the one where every service reports itself perfectly healthy and no user can reach it. So the measurement point sits as far toward the user as you can get it, ideally with client-side telemetry as a cross-check. And **requests are not users.** Failures cluster --- by customer, by region, by shard --- so 99.95% of *requests* can coexist with a large fraction of *users* having seen at least one failure, which is what actually generates the complaints that contradict your green dashboard. For a payment path I would want the SLI counted per **user journey**, or at minimum a companion metric for the proportion of customers who experienced any failure. The trap running through all four is the same: an SLI is a **definition** you negotiate, not a metric you discover, and the failure mode is a number everyone agrees on that nobody has checked means what they think.
...
Int: This service does ten requests a minute. Does any of that arithmetic still work?
No, and it is worth saying so directly rather than shipping the formula and letting it misfire, because **burn-rate alerting is a statistical estimate and low traffic destroys the statistics.** At ten requests a minute, a single failed request inside a one-hour window is a **0.17% error rate**, and inside a five-minute confirmation window it is **2%** --- so one transient failure can trip a 14.4x burn-rate rule that was designed to mean "a serious incident is underway." You get paged for `n=1`. Tighten it to avoid that and the slow-burn rule becomes incapable of ever firing. Neither setting is a tuning problem; the event rate simply cannot support the inference. So there are four honest responses and I would pick between them explicitly. **Lengthen the windows**, accepting slower detection in exchange for a sample size that means something --- usually the right first move. **Require a minimum event count** before a rule may fire, so no alert can trigger on one or two failures regardless of what the ratio says; this is the single highest-value guard and it is frequently missing. **Aggregate related low-traffic services into one SLO**, since ten endpoints at ten requests a minute are collectively a service with a usable event rate, and the user does not care which one failed. Or **use synthetic probes** to manufacture a traffic floor, which additionally fixes the blind spot where nobody is asking at 3am and an outage is therefore invisible. And then the trap that makes this genuinely dangerous rather than merely noisy: in most implementations the SLI is a **ratio**, so **zero traffic and perfect success are the same arithmetic** --- `0/0`, which usually renders as 100% or as no data, neither of which pages anyone. A failure severe enough to stop requests arriving at all therefore reads as **flawless availability**, and the burn-rate alert that was supposed to be your safety net goes quiet exactly when everything is broken. Which is why a **traffic-volume alarm** sits alongside the burn-rate rules: the absence of requests has to be a firing condition in its own right, not a gap in a graph.
...
### CURVEBALL | over-delivery | Your service has held 99.99% for twelve straight months against a 99.9% SLO. The team is proud of it. Is anything wrong?

Task: Kill both wrong answers, then say what a permanently-unbinding SLO is actually telling you.
Model: Two wrong answers to bury first. It is **not** a licence to go break something --- the budget is a **ceiling, not a quota**, and unspent budget is headroom, not waste. But it is also not simply a win to celebrate, because **an SLO that has never once bound a decision is not doing its job**: a number that cannot constrain anything is decoration, which is the same failure as having no error-budget policy, just wearing a nicer dashboard. Twelve months of never approaching the line is *evidence*, and it means one of three things --- I would disambiguate rather than assume. **(1) The SLO is simply set too loose**: 99.9% is not what we actually intend to provide, so it never informs a trade-off. **(2) We are over-invested in reliability** relative to what users need --- reliability is bought with engineering time and money, and if we never spend the budget on the risks it exists to *license* (the deferred migration, the aggressive rollout, the experiment), we have been buying a nine nobody asked for and paying for it in velocity. **(3) The dangerous one: our dependents have silently calibrated to the observed 99.99%, not the promised 99.9%** --- Hyrum's Law applied to reliability, where callers build against actual behaviour rather than the contract. My *effective* commitment has quietly drifted above my stated one, so the day I legitimately operate at 99.9% --- which I am entitled to --- I break teams who assumed more. The move is to **renegotiate the number, not manufacture failure**: decide deliberately whether 99.99% is the real target (then raise the SLO, fund it, and let dependents rely on it) or whether 99.9% genuinely suffices (then say so out loud, tell dependents to build for it, and start spending the headroom on the risk it was meant to buy). The output is a decision about what number we mean --- never an outage.
Int: So raise it to 99.99%, since that is what you are already delivering?
Not reflexively --- and this is exactly where the trap is, because raising an SLO is not a reward, it is a **permanent commitment to keep paying for that level**. Each nine costs roughly an order of magnitude more, and the allowance collapses fast: 99.9% is about **43 minutes** of badness a month, 99.99% is about **4 minutes**, and 99.999% is about **26 seconds** --- and you cannot defend four minutes a month with effort or care, only with **architecture**: redundancy, automated failover, remediation with no human in the loop. So the test is **demand-side, not supply-side**: does anyone actually *need* 99.99%, and will the business fund it? If the extra nine is a happy accident of a quiet year and modest load, formalising it converts **luck into an obligation** that someone now has to defend at 3am --- and it will run into the **dependency ceiling** regardless, because I cannot promise more availability than the things I synchronously require to answer a request. So the usual right answer is the reverse of the flattering one: keep the honest 99.9%, state it plainly so dependents stop assuming more, and redirect the surplus into shipping. I raise the SLO only when a real consumer requires that level and someone will pay for it --- otherwise I have quietly bought a harder job with no buyer.
