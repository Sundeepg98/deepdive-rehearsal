...
Partly, and it's ugly enough that most teams don't --- but first, the version people reach for does not work. You **cannot** build this on the nullable `deleted_at`: a composite FK against `(parent_id, parent_deleted_at)` is checked under **MATCH SIMPLE**, the default in Postgres and the only semantics MySQL implements, and MATCH SIMPLE **skips the check entirely when any column of the key is NULL**. A live parent is exactly the NULL case, so the constraint never fires --- you could insert a child against a `parent_id` that has never existed and it would pass. That is *strictly weaker* than the plain FK it replaced, because it stopped guaranteeing even existence. MATCH FULL doesn't rescue it either: one-column-null-one-not raises an error, so every live reference would be rejected. It is the same NULL rule the partial-index trick exploits three cards up, and here it bites you. What *works* is mirroring a **NOT NULL** liveness value instead of the nullable timestamp: give the parent `is_live boolean NOT NULL` with `UNIQUE (id, is_live)`, give the child a `parent_is_live boolean NOT NULL CHECK (parent_is_live)`, and make the FK composite against `(id, is_live)`. Every column is populated, so the constraint is always checked, and soft-deleting a parent that still has live children is **rejected** --- which is `RESTRICT`, enforced by the database instead of remembered by a developer. The price is that the decision is forced at delete time, and most teams would rather not. So the honest engineering answer is usually: accept that the FK guarantees **existence**, enforce **liveness** in the application at insert time, and add a **reconciliation check** --- a scheduled query for live children of deleted parents --- so the violation is at least **detected** even though it isn't **prevented**. You replaced prevention with detection, and you should say so out loud.
...
You can push liveness back into the database, but **not** on the nullable column: a composite key on `(parent_id, parent_deleted_at)` is never checked at all, because MATCH SIMPLE skips a foreign key whose columns include a NULL --- and a live parent is precisely the NULL case, so it is weaker than the plain FK it replaced. It works only against a **NOT NULL** liveness value (`is_live` on the parent, `UNIQUE (id, is_live)`, the child's copy `CHECK`ed to true), and what it buys is `RESTRICT`: soft-deleting a parent with live children is rejected. Most teams don't want that decision forced at delete time, so they enforce liveness in the application and add a reconciliation query that finds live children of deleted parents. You have traded **prevention for detection**, and you should say that out loud rather than pretend the FK still protects you.
...
- The honest fix | trade | "You can force it with a composite FK carrying the parent's liveness --- but only against a NOT NULL liveness column, never the nullable `deleted_at`, because a NULL anywhere in a foreign key means MATCH SIMPLE never checks it at all. Done right it gives you RESTRICT: you can't soft-delete a parent that still has live children. Most teams don't want that at delete time, so: enforce liveness in the app, and add a reconciliation query for live children of deleted parents. **Prevention traded for detection.**"
...
Model: **These are two different requirements that people collapse into one mechanism, and separating them is most of the design.** "Undoable" is a **product** feature --- a user deleted something and may want it back --- so it wants **soft delete**: a `deleted_at` column, the row still present, and the filter enforced **structurally** in one layer rather than remembered at each call site. "Legally erasable" is a **compliance obligation** with the opposite requirement --- the data must genuinely cease to exist --- so it cannot be the same mechanism, because a soft-deleted row is a row you are still holding. So I'd build three paths, not one. The **reversible window**, soft-deleted and restorable, bounded by a stated retention period. A **purge job** that hard-deletes everything past that retention, which is what stops "undoable" from silently meaning "kept forever." And a **separate erasure path**, triggered by a legal request rather than by a user clicking delete, that acts **immediately** and does not wait for the retention clock --- because the obligation has its own deadline, typically thirty days, that has nothing to do with your product's undo window. Keeping those three visibly distinct is what lets you answer the two questions that actually get asked: *can the user get it back*, and *is it gone*.
Int: You offered "or anonymize" as an alternative to hard delete on the erasure path. Is the anonymized version genuinely erased?
Usually not, and this is where engineering convenience quietly gets sold as a legal position --- so it is worth being precise. What people typically build is **pseudonymization**, not anonymization: the name field is overwritten, and the row survives with its primary key, its foreign keys, its timestamps and its full behavioural history. If that record can still be **re-linked to a person** --- by joining it to another table you kept, or simply by recognizing it --- it remains personal data, and the obligation is not discharged. The test worth applying is **singling out**: can you still pick this individual out of the dataset? A row that is unique on its quasi-identifiers is not anonymous no matter what is in the name column, and in practice the identity leaks through the parts nobody thinks of as identifying --- the exact signup timestamp, the order history, the device or IP, a support ticket. So if erasure is the requirement, either **genuinely delete the row**, or irreversibly destroy the **linking identifiers** *and* coarsen the quasi-identifiers, and be able to state which of those you did and why it is sufficient. It is also worth being honest about *why* anonymize gets chosen: almost always because a hard delete breaks **foreign keys** and someone wants the analytics or the order history to keep working. That is a real engineering problem and a legitimate thing to solve --- it is just not a legal argument, and the two should not be conflated in the same sentence. The design that makes this tractable is to **separate the personal attributes into their own table keyed by subject**, so the erasure is a delete of one row and the surrounding non-personal facts keep referential integrity on a subject id that no longer resolves to anybody. Decide that on day one, because retrofitting it means finding every place a name, an email or an address leaked into a table that was not supposed to hold one.
...
Model: **The core problem is that a soft delete is a write, not a delete --- so the table only ever grows.** Two million of those ten million rows are logically gone and physically present, and every unqualified scan, every index that does not know about `deleted_at`, and every planner row estimate is working against the full ten million. So the reads have to be kept off the dead weight deliberately. A **partial index** --- built `WHERE deleted_at IS NULL` --- is the main lever: it indexes only the live fifth, so it is smaller, stays hotter in cache, and gives the planner an accurate picture of the set that queries actually touch. The catch to say out loud is that the planner only uses it when the query's predicate **matches the index's predicate**, so this only pays if the live filter is applied consistently, which is the argument for enforcing it in one structural layer rather than per query. Then a **purge job** past the retention window caps the deleted set, so the bloat reaches a steady state instead of growing forever --- because "we can restore it" and "we keep it indefinitely" are different promises and only the first one was made.
Int: When you soft-delete a parent row, what happens to its children? The database cascades a `DELETE`; it cannot cascade a predicate.
That is the structural cost of soft delete and the part that gets underestimated, because `ON DELETE CASCADE` is doing real work that you have just opted out of. Once the parent is only *marked* deleted, its children are untouched and fully live, still pointing at it --- so the invariant "a live child implies a live parent" stops being enforced by the database and becomes **application-enforced across every relationship in the schema**. That is the forgotten-filter bug again, one level removed and much harder to see, because the child table looks perfectly consistent on its own. There are three coherent answers and the failure is not picking one. **Cascade the soft delete explicitly** --- stamp `deleted_at` on the children in the same transaction --- which makes deletes more expensive and complicates restore, because you must now distinguish a child deleted *with* its parent from one the user deleted earlier and does not want back. **Filter through the join** --- children carry no flag and every query checks the parent's liveness --- which keeps writes cheap and puts the burden on every read path that touches the relationship. Or **do not soft-delete children at all** and treat the parent's state as the single source of truth, which is the cleanest when children have no independent lifecycle. Any of those works; a schema where different tables silently made *different* choices does not, and that is the actual thing to check for. The operation that exposes all of this is **restore**, which is worth designing before you need it: restoring a parent must bring back exactly the children that went with it, and a bare `deleted_at` timestamp cannot tell you which those were --- rows deleted a second apart are indistinguishable. So the delete should record **why**, with a delete-batch or correlation id, turning restore into "reverse batch X" instead of a guess based on timestamps. And if the cascade question has no consistent answer across your schema, that is usually the signal that soft delete is being applied more broadly than it earns.
...
Model: **The plain unique index still counts their soft-deleted row**, so as far as the database is concerned the email is taken. Replace it with a partial unique index over `deleted_at IS NULL` so uniqueness applies only to live rows, freeing the deleted user's email for reuse --- the classic soft-delete unique-constraint fix.
...
You can, and it's ugly enough that most teams don't --- but get the mechanism right, because the obvious version is a no-op. Carrying `parent_deleted_at` on the child and making the FK composite against `(parent_id, parent_deleted_at)` **does not enforce anything for a live parent**: the child stores NULL there, and under MATCH SIMPLE a foreign key with any NULL column is not checked at all, so you could reference a parent that never existed. It is weaker than the plain FK it replaced. The working shape mirrors a **NOT NULL** liveness value --- `is_live boolean NOT NULL` on the parent with `UNIQUE (id, is_live)`, a `parent_is_live boolean NOT NULL CHECK (parent_is_live)` on the child, FK against `(id, is_live)` --- and then soft-deleting a parent with live children is **rejected** outright, which is `RESTRICT` bought at the database level. The price is that the decision is forced at delete time. So the honest engineering answer is usually: enforce liveness in the **application** on every insert that takes a parent id, and add a **reconciliation query** that finds live children of deleted parents and alerts. You have traded **prevention for detection** --- and the maturity is saying that out loud rather than pretending the FK still protects you.
