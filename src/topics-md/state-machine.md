...
Int: A `ship` event arrives for an order already in `shipped`. Your table refuses it. Was that right?
I genuinely cannot tell from the table, and that is the important gap --- because **two completely different situations produce the identical observation**. It is either a **duplicate delivery** of a transition that already succeeded, in which case refusing it is wrong and the caller should get the original success, or it is a **second, distinct attempt to ship** --- a retry from a different actor, a double-clicked operator, a bug --- in which case refusing it is exactly right. The `(state, event)` pair is the same in both, so the table cannot separate them, and whichever behaviour I hardcode is wrong half the time. This matters because the surrounding system is at-least-once: commands get redelivered, and a machine that treats every redelivery as an illegal move will refuse legitimate operations under precisely the conditions that cause redelivery. The resolution is that **the transition needs an identity of its own**. Each attempt carries a transition or request id, and applied transitions are recorded, so a repeat of the *same* id returns the original outcome and reports success, while the same *event* under a *different* id against an incompatible state is properly refused. That is the same idempotency-key move used on any side-effecting operation, applied to the transition rather than the entity. Two consequences worth stating. The refusal must be an **explicit, informative response** --- current state plus a reason --- never a silent no-op, because a silent no-op makes a caller's bug indistinguishable from success and the entity simply stops progressing while everything reports fine. And the principle underneath: **a state machine encodes what is legal, not what has already happened.** You need both facts, and conflating them is how a correct-looking table produces "the retry was rejected" incidents that are very hard to read from the code.
Int2: The hot entity is now the bottleneck --- 500 moves/sec want to hit one row. Now what?
Measure the real commit latency before anything else, because the ceiling I sketched above is a formula and 500/s only means something against the number this database actually produces. Say it lands where I guessed. Then 500/s isn't over the cliff; it's a **strictly serial lane running at half capacity**, which is the more dangerous place to be, because there's no headroom for a commit-latency spike and optimistic CAS doesn't plateau gracefully --- past the knee the losers re-read and retry, which *adds* load and collapses throughput. So with the real number in hand: **shard the entity** if the semantics allow (split the lifecycle into independent sub-entities that transition separately), or **batch** its moves through a single-writer lane per entity key, which converts contention into deliberate serialization and removes the retry amplification. If they genuinely must all hit one row, I name the measured ceiling and design around it --- I don't pretend one row scales.
...
Int: The author gets the first approval, then edits the document, then gets the second. Your guards all passed. What did you just publish?
A document that **nobody approved** --- and this is the failure that makes four-eyes controls look sound while being trivially defeatable, so it is worth being blunt about it. My guards enforce that two distinct, MFA-verified principals each performed an approval action, and every one of those checks passes here. What they never established is that the two people approved **the same content**. Approver one signed off on a document, the author changed it, approver two signed off on a different document, and the machine happily reached `approved` holding text that no pair of eyes ever jointly saw. In an adversarial version the author does not even need a second account: they need only patience, and an edit between the two approvals. The fix is that **an approval must bind to a version, not to an entity** --- the transition records a **content hash** of exactly what was approved, and the guard on `-> approved` requires every recorded approval's hash to match the document's current hash. Any edit to a document in `submitted` or `approved_once` therefore **invalidates the approvals already collected** and returns it to `draft`, which is the correct and slightly annoying behaviour: annoying is the point, since the alternative is an unreviewed publish. That also fixes the audit story, because "Priya approved this" is only a meaningful record if it says *which version* she approved --- otherwise the log proves an action occurred and proves nothing about what was reviewed. And the generalisation is worth carrying beyond this card: **an approval is a statement about a specific artifact at a specific moment, so it must be bound to that artifact's identity** --- the same reason a signature covers a hash of the payload rather than merely asserting that signing happened.
...
Because the cap is a property of **this entity's retry history**, and it has to **survive restarts** --- if the count lived in memory, or on one transition record, a process restart resets it to zero and the poison item retries forever, which is the exact failure the cap exists to prevent. On the entity, the guard on `failed -> retry` reads it, compares against the cap, and diverts to `dead`. Putting it there buys two more things worth naming. It makes the retry schedule **queryable**: with `attempts` and `next_retry_at` as columns, "what is stuck, how hard have we tried, and when does it next move" is a `SELECT` --- and an in-memory retry loop can never answer that question, which is why stuck work in such systems is invisible until a customer asks. And it makes the cap **enforceable by whoever performs the transition**, rather than depending on the retrying process being the same one that failed. Two details decide whether it actually works in production. **The counter must be reset on success**, and on a deliberate re-drive --- otherwise an entity that fails transiently once a month accumulates attempts across entirely unrelated episodes and eventually dies for no reason at all, which presents as a mystery months after the code was written. And ideally attempts are counted **per failure class**, because a timeout against a downstream service and a malformed payload deserve different budgets: the first should be retried generously since it will likely succeed, the second will never succeed and should reach `dead` almost immediately. Then the part people skip: **`dead` needs an owner and a re-drive path.** A terminal state with nothing behind it is a graveyard nobody reads, so it wants an alert, a queue, and a supported way to fix the cause and push the entity back to `pending` --- because most poison items are poison for a reason you can repair.
...
No --- the serialization is **logical**, not thousands of physical queues. I partition by entity key, so a queue or actor exists only for the genuinely *active* hot keys, and the cold majority --- which is nearly all of them --- keeps transitioning directly with a plain CAS and never touches the mechanism at all. The discipline lives in **routing by entity key**, not in a queue per row. But the follow-through is where this gets real, and there are three parts I would not hand-wave. **Which keys get a lane has to be detected, not configured**, because "the hot entity" changes --- it is a top-K or heavy-hitters problem over the transition stream, exactly as it is for a hot shard, with promotion when a key crosses a threshold and demotion when it goes quiet; a hardcoded list is stale within a week. **Routing must be consistent and owned**, meaning the same key always reaches the same lane, which makes this a placement problem with all the usual properties: a rebalance has to hand a key over without two lanes briefly owning it, and a lane that dies mid-batch must have its in-flight work recovered rather than silently dropped --- so lanes need supervision and restart, otherwise one dead lane means one entity stops moving forever while everything else looks healthy. And **the trade has to be stated**: a single-writer lane does not create throughput, it converts **contention into queueing latency** and introduces head-of-line blocking within that key. That is normally the right trade --- predictable latency and no retry amplification beats a CAS storm collapsing under its own losers --- but it is a trade, and the honest version of this answer names it rather than presenting the lane as free.
...
Int: You said you'd upgrade reactively. But by the time the scattered checks are drifting, production is full of rows in states your new machine says are impossible. How does that migration actually go?
That is the real cost of the reactive strategy and I would rather price it than pretend it is free --- the hard part is never writing the table, it is that **you cannot deploy a machine that rejects the data you already have.** Live rows will be sitting in combinations the idealized design says cannot exist, produced by the very drift that prompted the upgrade, plus manual database edits, half-finished migrations and bugs nobody found because nothing was checking. So the sequence matters. **Measure first**: query the actual distribution of states, and if there is any history, the transitions that genuinely occurred --- the real lifecycle is reliably larger and uglier than anyone's mental model, and discovering that from a query is much cheaper than discovering it from a rejected write. **Make the first table a description, not an aspiration**: it should encode what the system actually does today, including the paths you dislike, because a machine that rejects reality on day one is simply an outage. You tighten it afterwards, deliberately, one transition at a time. **Then run it in shadow mode** --- evaluating and logging what it *would* have refused without refusing anything --- which converts the risky part of the migration into data you can read for a week, and is the same observe-then-enforce discipline you would use for a rate limit. **Then resolve the illegal rows explicitly**: correct them where you can, and where you cannot, grandfather them into a named state like `legacy_unknown` rather than quietly pretending they are `pending`, because an honest state you can query beats a comfortable lie. **Only then enforce.** And this whole cost is still worth paying rather than designing the machine on day one, because an eight-state lifecycle drawn before you understand the domain is a *wrong* machine --- you will migrate anyway, just from a design nobody validated instead of from data. The difference is that this migration is driven by measurement, which makes it bounded.
