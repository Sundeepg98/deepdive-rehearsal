...
var TOPIC_AUTHZ_SYS = {
  ...
  pivots: [
    ...
    { q: "Where does the tenant-scoped data actually live, and how does the scope stay fast?", chip: "\u2192 Attribute Store (6)",
      a: "The predicate lands on the <b>data layer</b>: <code>tenant_id</code> leads every index so the scoped query is a range seek, and the store (Postgres RLS, or DynamoDB partition keys) enforces the boundary. Authorization decides the scope; the data layer makes it both <i>enforced</i> and <i>efficient</i>. This is exactly where a per-tenant attribute store lives &mdash; every tenant&rsquo;s values under a tenant-scoped partition &mdash; so the isolation model (silo vs pool) is a storage-design decision as much as a security one." },
    { q: "One tenant&rsquo;s traffic spikes &mdash; how do you keep it from starving the others?", chip: "\u2192 Notifications (5)",
      a: "<b>Per-tenant fairness</b>: rate limits, quotas, fair queuing, and per-tenant metrics to attribute load &mdash; the same per-tenant rate-limiting and fair-scheduling a notification system needs so one tenant&rsquo;s send burst can&rsquo;t starve another. Isolation isn&rsquo;t only about <i>data</i> &mdash; it&rsquo;s about <i>resources</i> too. A pooled system where one tenant can consume another&rsquo;s share has failed isolation just as surely as one that leaks rows. The whale gets its own lane, not the whole road." },
