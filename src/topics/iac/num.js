...
var TOPIC_IAC_NUM = {
  ...
  compute:function(vals, fmt){
    ...
    var sOk = S > 0;                /* an emptied state count divided by zero */
    var blast = sOk ? R / S : 0;
    var total = R * E;
    var giantBlast = R;
    var planSplit = sOk ? (R / S) / 100 * P : 0;
    var planGiant = R / 100 * P;
    var stateFiles = S * E;
    return [
      { k:'Blast radius (resources / state)', v:sOk ? fmt.n(blast) : 'n/a', u:'resources', n:'the most resources one bad apply can take down \u2014 this is why you split state by risk and change-rate.', over:true },
      ...
      { k:'Plan time per split state', v:sOk ? fmt.n(planSplit) : 'n/a', u:'seconds', n:'refresh + diff for one bounded state \u2014 fast, because plan only walks that state\u2019s resources.' },
