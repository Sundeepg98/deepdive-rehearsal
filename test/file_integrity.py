...
def check_js_file(path):
    ...
    for match in re.finditer(r'class\s+(\w+)\s+extends\s+\w+\s*\{', content):
        ...
        for ch in content[start:]:
            ...
            errors.append(f"  ❌ Class '{name}' is NOT properly closed (truncated?)")
    ...
    for name in class_names:
        ...
        if not re.search(pattern, content):
            errors.append(f"  ❌ customElements.define missing for class '{name}'")
...
def main():
    ...
    if errors:
        ...
        print(f"✅ All {len(js_files)} files passed integrity check")
        sys.exit(0)
