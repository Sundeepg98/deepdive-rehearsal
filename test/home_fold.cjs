 ...
 * chip list's top is a function of THREE independent things -- how far the hero question wraps
 * (23px on the shortest probe in the bank, 164px on the longest, at a 7-line clamp), which verdict
 * class the record lands in (the single-thin-rail sentence is 62px, the two-thin one 80px), and
 * whether the record has weak topics (one 66px practice bar or two, 144px). Across those the chip
 * list lands anywhere from 30px inside the band to 188px below it, and no compaction closes the
 * far end without deleting content. So the contract names a DISJUNCTION: the record's triage is on
 * the first screen either as the chip list or as the act addressed to that record, and this file
...
const SHAPES = [
  ['one-thin x 1 bar x short hero', { pct: ONE_THIN_1, weakN: 0, hero: 'short' }, 'one-thin', 1],
  ['one-thin x 1 bar x LONG hero', { pct: ONE_THIN_1, weakN: 0, hero: 'long' }, 'one-thin', 1],
  ['one-thin x 2 bars x short hero', { pct: ONE_THIN_2, weakN: 12, hero: 'short' }, 'one-thin', 2],
  ['one-thin x 2 bars x LONG hero', { pct: ONE_THIN_2, weakN: 12, hero: 'long' }, 'one-thin', 2],
  ['two-thin x 1 bar x short hero', { pct: TWO_THIN_1, weakN: 0, hero: 'short' }, 'two-thin', 1],
  ['two-thin x 1 bar x LONG hero', { pct: TWO_THIN_1, weakN: 0, hero: 'long' }, 'two-thin', 1],
  ['two-thin x 2 bars x short hero', { pct: TWO_THIN_2, weakN: 12, hero: 'short' }, 'two-thin', 2],
  ['two-thin x 2 bars x LONG hero', { pct: TWO_THIN_2, weakN: 12, hero: 'long' }, 'two-thin', 2],
  ['cold (nothing graded anywhere)', null, 'cold', 1],
];
...
const MUTANT_SHAPE = 'two-thin x 2 bars x LONG hero';
...
(async () => {
  ...
  for (const [name, cfg, wantVerdict, wantBars] of SHAPES) {
    ...
    /* A VIEWPORT THIS CHECK BELIEVES, and a shape it believes too. A matrix whose rows have
       silently collapsed onto one cell reports four greens for one measurement. */
    ok('[' + name + '] the viewport really is 390x844 and the record really is the shape this row names',
      f.vp.w === 390 && f.vp.h === 844 && f.verdictClass === wantVerdict && f.bars === wantBars,
      JSON.stringify({ vp: f.vp, verdict: f.verdictClass, want: wantVerdict, bars: f.bars, wantBars }));
    ...
    console.log('        hero ' + f.heroH + 'px  verdict ' + f.verdictClass + '  bars ' + f.bars
    ...
    if (name === MUTANT_SHAPE) {
      ...
      if (!planted) {
        aborted = 'THE PLANT COULD NOT LAND: there is no #home .hm-practicem to move, so the '
        ...
        if (bad.actIn || bad.chipIn) {
          aborted = 'SELF-TEST UNDETECTED: with .hm-practicem returned to the END of the column -- '
            + 'the position it occupied before W1.5, measured at top 2136 -- the assertion still '
            + 'passed. act ' + JSON.stringify(bad.actBox) + ' in=' + bad.actIn + ', first chip '
            + JSON.stringify(bad.chip1) + ' in=' + bad.chipIn + ', band ' + JSON.stringify(bad.band)
  ...
  console.log('\n  1 planted mutant detected (the practice block returned to the END of the column, '
    + 'where it measured top 2136 before W1.5 -- both carriers leave the band and the arm goes red)');
  console.log('HOME FOLD: PASS  (' + (SHAPES.length * 4) + ' assertions across ' + SHAPES.length
    + ' records at 390x844 -- verdict class x bar count x hero wrap, each asserted against a band '
