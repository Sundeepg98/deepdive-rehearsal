...
// Generate topics/<id>.js -- the include manifest that pulls a topic's slices in order.
function writeBundle(topicsDir, id) {
  ...
  for (const s of BUNDLE_SLICES) {
    if (fs.existsSync(path.join(dir, s + '.js'))) lines.push('<!--@build:include topics/' + id + '/' + s + '.js-->');
  }
  fs.writeFileSync(path.join(topicsDir, id + '.js'), lines.join('\n') + '\n');
}
...
function writeRegistry(registryPath, topics) {
  ...
  for (const t of topics) lines.push('<!--@build:include topics/' + t.id + '.js-->');
    ...
    async buildStart() {
      ...
      for (const t of topics) {
        await compileTopic(path.join(srcDir, t.file), {
          index: t.index || 1, total, outDir: path.join(topicsDir, t.id),
        });
